...
(function ($) {
    var ms = $.wemoTicker,		
        ...
        defaults = {
			...
			timeWheels: 'hh',           			
            timeFormat: 'hh:ii A',  
			thresholdFormat: 'w '+wemo.items['Watts'],
			costPerFormat: '$a.bcd',
        ...
        preset = function (inst) {
            ...
            var s = $.extend({}, defaults, html5def, inst.settings),
                ...
                ampm = tord.match(/a/i),
                hampm = tord.match(/h/),
				hformat = p == 'datetime' ? s.dateFormat + s.separator + s.timeFormat : p == 'time' ? s.timeFormat : p== 'threshold' ? s.thresholdFormat : p== 'costPer' ? s.costPerFormat : s.dateFormat,
                defd = new Date(),
			...
			if (p.match(/threshold/i)) {	
                w = {};
				w[s.wattsText] = {};
				for (i=2; i<101; i+=( i<50 ? stepW :stepW*5 )) {
					 w[s.wattsText][i]=i;					
				}	
                wheels.push(w);
			}
			
			if (p.match(/costPer/i)) {
                w = {};           
				w[s.costText[0]] = {};
				for (i = 0; i < 10; i += 1) {
					w[s.costText[0]][i] = i;
				}
				w[s.costText[1]] = w[s.costText[0]];
				w[s.costText[2]] = w[s.costText[0]];
				w[s.costText[3]] = w[s.costText[0]];
                wheels.push(w);
                width = 50;
			}
            ...
            return {
                ...
                headerText: function (v) {
					if (hformat=="w Watts") {
						return ms.formatThreshold(hformat, inst.temp);
					} else {
						return ms.formatCost(hformat, inst.temp);					
					}			
                },
                /**
                ...
                */
                parseValue: function (val) {				
					var result=[];			
					var id=$('button:contains('+val+')').attr("id");
					if (id=="set_cost_per") {	
						val=val.replace(/[$,.]/g, "");
						for (var i=0; i<4; i++) {
							result.push(val[i]);
                ...
                methods: {
                    ...
                    */
                    getDate: function (temp) {
                        var inst = $(this).wemoTicker('getInst');
                        if (inst) {
	...
	$.each(['date', 'time', 'datetime', 'threshold', 'costPer'], function (i, v) {
        ms.presets[v] = preset;
        ms.presetShort(v);
    });
    ...
    */
	...
	ms.formatCost = function (format, value) {	
		res=format.replace("a",value[0]).replace("b",value[1]).replace("c",value[2]).replace("d",value[3]);	
		return res;
	};
	
	ms.formatThreshold = function(format, value) {
		res=format.replace("w",value[0]);	
		return res;
	};
