(function () {
define('text!widgets/wemo_lighting/css/main.css',[],function () { return '\n.wemo_lighting .icon {\n    background-size: 100% 100%;\n    background: url(data:image/png;base64,) no-repeat;\n    background-size:cover;\n}\n.wemo_lighting .bulb_mz100,\n.wemo_group .bulb_mz100,\n.wemo_lighting .default.bulb_mz100 {\n    background: url(data:image/png;base64,) no-repeat;\n    background-size:cover;\n}\n.wemo_lighting .bulb_classic_a60_tw,\n.wemo_group .bulb_classic_a60_tw,\n.wemo_lighting .default.bulb_classic_a60_tw {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_flex_rgbw,\n.wemo_group .bulb_flex_rgbw,\n.wemo_lighting .default.bulb_flex_rgbw {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_gardenspot_rgb,\n.wemo_group .bulb_gardenspot_rgb,\n.wemo_lighting .default.bulb_gardenspot_rgb {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_lightify_a19_tunable_white,\n.wemo_group .bulb_lightify_a19_tunable_white,\n.wemo_lighting .default.bulb_lightify_a19_tunable_white {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_lightify_flex_rgbw,\n.wemo_group .bulb_lightify_flex_rgbw,\n.wemo_lighting .default.bulb_lightify_flex_rgbw {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_lightify_gardenspot_rgb,\n.wemo_group .bulb_lightify_gardenspot_rgb,\n.wemo_lighting .default.bulb_lightify_gardenspot_rgb {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_iqbr30,\n.wemo_group .bulb_iqbr30,\n.wemo_lighting .default.bulb_iqbr30 {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .bulb_connected_a-19_60w_equivalent,\n.wemo_group .bulb_connected_a-19_60w_equivalent,\n.wemo_lighting .default.bulb_connected_a-19_60w_equivalent {\n    background-image: url(data:image/png;base64,);\n    background-size:cover;\n}\n.wemo_lighting .smart-tile .powerState {\n    background-size: 100%;\n}\n.wemo_lighting .sliderContainer {\n    width: 100%;\n    height: 9em;\n    background: url(data:image/png;base64,) no-repeat 3% 50%, url(data:image/png;base64,) no-repeat 97% 50%;\n    background-size: 7%;\n    padding: 20px 10%;\n}\n.wemo_lighting .colorPicker {\n    width: 100%;\n    display:flex;\n    padding: 15px 0;\n    align-items: center;\n    overflow: hidden;\n}\n.wemo_lighting .colorPicker .fa {\n    float: right;\n    color: #dddddd;\n    font-size: 22px;\n    margin-right: 20px;\n}\n.wemo_lighting .colorPicker .tempPicker {\n    width: 3em;\n    height: 3em;\n    border-radius: 50%;\n    margin: 0 4em;\n    float: left;\n}\n.wemo_lighting .colorPicker .colorTempTitle {\n    font-size: 17px;\n    color: #444444;\n    flex: 1;\n}\n.wemo_lighting .content {\n    overflow-y: auto;\n}\n.wemo_lighting .drawerContent {\n    height: 100%;\n}\n.wemo_lighting .colorPicker, .wemo_lighting .sliderContainer, .wemo_lighting .sleepFader {\n    border-bottom: 1px solid #BBB;\n}\n.wemo_lighting .sleepFader {\n    width: 100%;\n    height: 9em;\n    background: #eaeaea;\n    position: relative;\n}\n.wemo_lighting .sleepFader span {\n    position: relative;\n    font-size: 2em;\n    display: block;\n    padding: 0.5em 0 0 1em;\n}\n.wemo_lighting {\n    font-family: "Roboto";\n}\n.wemo_lighting .sleepOptions .radBtn {\n    background: url(data:image/png;base64,) no-repeat;\n    background-size: 100%;\n    display: block;\n    height: 2.4em;\n    width: 2.4em;\n    margin: 0 auto;\n}\n.wemo_lighting .sleepOptions span.active {\n    background: url(data:image/png;base64,) no-repeat;\n    background-size: 100%;\n}\n.wemo_lighting .sleepOptions {\n    width: 100%;\n    height: 10em;\n    padding: 2em 0 0 0;\n}\n.wemo_lighting .sleepOptions .parent, .wemo_lighting .sleepOptions #custom {\n    float: left;\n    width: 10%;\n    margin: 0 7%;\n    font-size: 1.6em;\n}\n.isPeekaboo .smart-drawer.wemo_lighting {\n    height: 9em;\n    overflow: hidden;\n}\n.isPeekaboo .smart-drawer.wemo_lighting section.content {\n    overflow-y: hidden;\n}\n.wemo_lighting .sleepOptions #custom {\n    width: 20%;\n    margin: 0 0 0 6%;\n}\n.wemo_lighting .sleepOptions .parent:first-child {\n    margin-left: 4%;\n}\n.wemo_lighting .sleepOptions .information {\n    margin-bottom: 0.5em;\n    display: block;\n    text-align: center;\n}\n.wemo_lighting .sleepOptions .button {\n    width: 100%;\n    display: block;\n    border: 1px solid #73D34A;\n    padding: 0.8em;\n    text-align: center;\n    font-size: 0.8em;\n    background-color: #f7f7f7;\n    color: #000;\n}\n.wemo_lighting #sleepButton {\n    width: 90%;\n    height: 2em;\n    background: #eaeaea;\n    display: block;\n    text-align: center;\n    padding: 0.3em;\n    font-size: 3em;\n    color: #73D34A;\n    margin: 0.7em auto;\n    font-weight: bold;\n}\n.wemo_lighting #sleepButton.disableSleepFader {\n    background: #eee;\n    color: #ccc;\n}\n.wemo_lighting .form-list-select span.customSet {\n    background: #ace588;\n    color: #fff;\n}\n.wemo_lighting .row-tile .description #dimValue.active {\n    color: #73D44C;\n}\n.wemo_lighting paper-slider::shadow #sliderBar::shadow #activeProgress {\n    background-color: #ADE48C;\n}\n.wemo_lighting paper-slider::shadow #sliderKnobInner,\n.wemo_lighting paper-slider::shadow #sliderKnob {\n    width: 3em;\n    height: 3em;\n}\n.wemo_lighting paper-slider::shadow #sliderKnobInner {\n    background-color: #73D44C;\n}\n.wemo_lighting paper-slider {\n    width: 70%;\n    display: block;\n    top: 50%;\n    margin: 0 11%;\n    position: relative;\n    transform: translateY(-50%);\n}\n.wemo_lighting paper-slider::shadow #sliderContainer {\n    height: 3em;\n    width: 100%;\n    position: relative;\n}\n.wemo_lighting paper-slider::shadow #sliderBar {\n    top: 0;\n    height: 3em;\n}\n.wemo_lighting paper-slider::shadow #sliderBar::shadow #progressContainer {\n    border-radius: 25px;\n    background-color: #eaeaea;\n}\n.wemo_lighting paper-slider::shadow .ring > #sliderKnob > #sliderKnobInner {\n    background-color: #73D44C;\n    border: none;\n}\n.wemo_lighting paper-slider::shadow .ring #sliderBar {\n    left: 0;\n    width: 100%;\n}\n.wemo_lighting paper-slider::shadow .transiting #sliderBar::shadow #activeProgress {\n    transition: none;\n}\n.wemo_lighting paper-slider::shadow .ring.expand:not(.pin) > .bar-container,\n.wemo_lighting paper-slider::shadow .ring > .bar-container {\n    width: calc(100% + 0.1em);\n    left: 1.7em;\n}\n.wemo_lighting.smart-overlay header {\n    background: #73D44C;\n    height: 56px;\n}\n.wemo_lighting.smart-overlay header .back .title{\n    font-size: 18px;\n    font-weight: 300;\n    line-height: 56px;\n    color: white;\n    margin-left: 30px;\n}\n.wemo_lighting.smart-overlay header .back .backIcon {\n    position: absolute;\n    top: 8px;\n    left: 10px;\n    background-image: url(data:image/png;base64,);\n    background-repeat: no-repeat;\n    background-size: 12px 20px;\n    background-position: center;\n    padding: 20px;\n    width: 16px;\n    height: 16px;\n}\n.wemo_lighting .right-arrow {\n    font-size: 2.5em;\n    font-family: FontAwesome;\n    line-height: 100%;\n    top: 50%;\n    transform: translateY(-50%);\n    position: relative;\n}\n.wemo_lighting .right-arrow {\n    right: 5%;\n    float: right;\n}\n.wemo_lighting .right-arrow:before {\n    content: \'\\f054\';\n}\n.wemo_lighting.settings .tabs {\n    width: 100%;\n}\n.wemo_lighting.settings .tabs span {\n    width: 50%;\n    height: 100%;\n    display: block;\n    font-size: 2em;\n    padding: 15px;\n    position: relative;\n    float: left;\n    text-align: center;\n    text-transform: uppercase;\n}\n.wemo_lighting.settings .tabs .active {\n    border-bottom: 2px solid #73D44C;\n}\n.wemo_lighting.settings .content {\n    padding: 0;\n    margin: 0;\n    margin-top: 56px;\n}\n.wemo_lighting.settings header.specialCase {\n    border: none;\n    box-shadow: none;\n}\n.wemo_lighting #colorPicker, .wemo_lighting #temperaturePicker {\n    display: inline-block;\n    width: 100%;\n    height: 100%;\n    overflow: hidden;\n}\n.wemo_lighting header.specialCase + section.content #colorPicker,\n.wemo_lighting header.specialCase + section.content #temperaturePicker {\n    height: 91%;\n}\n.wemo_lighting .ID_container.checkChange {\n    padding: 1.5em 2.2em;\n    height: 14em;\n}\n.wemo_lighting .ID_container .checkBulb_container {\n    width: 100%;\n    height: 5.9em;\n    display: none;\n    line-height: 6.5em;\n}\n.wemo_lighting .checkBulb_container .find_bulb {\n    color: #73d44c;\n    font-size: 4.5em;\n}\n.wemo_lighting .ID_label.checkChange {\n    height: 2.5em;\n    line-height: 2.5em;\n}\n';});

define('widgets/wemo_lighting/js/model',['Cache'],
    /** @exports model class and instantiated model */
    function(Backbone) {
        'use strict';

        var mockPath = 'widgets/wemo_bulb/mocks/';

        var Model = Backbone.WemoDeviceModel.extend({
            initialize: function() {
                this.once('change:sleepFader', this.checkSleepFader);
            },
            /**
             * default data values
             */
            defaults: {
                'selectedTimer': 1,
                'customOption': '',
                'pickerView': 0,
                'activeCapability': 'colorControl',
                'defaultFriendlyName': '',
                "sleepFader": "0:0",
                "levelControl": "255:0",
                "colorTemperature": "0:0",
                "onOff": "0",
                "colorControl": "0:0:0",
                "defaultIcon": "file:///android_asset/www/img/new_icons/WeMo_LightBulb_@3x.png"
            },
            /**
             * validation fields, uses Backbone.validation
             */
            validation: {
                'onOff': {
                    required: true
                },
                'levelControl': {
                    required: true
                },
                'sleepFader': {
                    required: true
                },
                'colorControl': {
                    required: true
                },
                'colorTemperature': {
                    required: true
                }
            },
            /**
             *  set the native get command
             */
            getCommand: 'GetAttributes',
            /**
             *   set the native command
             */
            setCommand: 'SetAttributes',
            checkSleepFader: function() {
                var sleepFader = this.get('sleepFader');
                if (!sleepFader) {
                    sleepFader = '0:0';
                    this.set('sleepFader', sleepFader);
                }
            },
            /**
             * Conversion for levelControl variable - get method
             * @param {number} value level value
             */
            getLevelControl: function() {
                if (this.get('levelControl')) {
                    var value = this.get('levelControl').split(':')[0] ? this.get('levelControl').split(':')[0] : 255;
                    value = Math.round((value * 100) / 255);
                    return value;
                } else {
                    return 0;
                }
            },
            /**
             * Conversion for levelControl variable - set method
             * @param {number} value level value
             */
            setLevelControl: function(value) {
                value = Math.round((value * 255) / 100);
                this.set('levelControl', value + ':0');
            },
            saveAction: function(variable) {
                return this.save(variable);
            },
            /**
             * Object of product names and its corresponding view type:
             * 1- Temperature picker,
             * 2- Color picker,
             * 3- Temperature and color pickers
             */
            productNameObj: {
                'temperatureBulb': 1,
                'colorBulb': 2,
                'flexBulb': 3
            },
            /**
             * urls, statusUrl, and rulesUrl all allow dummy data to be loaded for demo/test
             * @returns {string}
             */
            url: function() {
                return mockPath + 'device.json';
            }
        });

        return {
            Model: Model,
            model: new Model() //we need to pass a shared model, but we want the base object available for unit testing
        };
    });

/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/tile',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  


  return "<header class=\"row-tile\">\n    <div class=\"tile1\">\n        <div class=\"gradient\"></div>\n        <span class=\"icon\" data-bind=\"iconURL\"></span>\n    </div>\n    <div class=\"tile2 description\">\n        <h1 data-bind=\"friendlyName\"></h1>\n        <span class=\"status\" id=\"dimValue\" data-bind=\"LevelControl\"></span>\n        <div class=\"drawerArrow closed\"></div>\n    </div>\n    <div class=\"tile3 power\"></div>\n</header>\n";
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_tile', t);
return t;
});
/* END_TEMPLATE */
;
define('extensions/ui/powerButton', ['backbone'], function(Backbone) {

    'use strict';


    var PowerButton = Backbone.View.extend({
        events: {
        },

        initialize: function(options) {
            _.extend(this, options);
            this.isRendered = false;
            this._state = -1;
        },

        setState: function(state){
            if(state === this._state) return;

            var self = this;
            var _setState = function(){
                self.$('.powerState').attr('class', 'powerState powerState-' + state);
                self._state = state;
            };

            _setState(state);
        },

        render: function(){
            if(!this.isRendered){
                this.$el.html('<div class="powerState powerState-0" data-bind="modeIcon" aria-live="polite" aria-atomic="true" role="button"><div class="powerStateSpinner"></div></div>');
                this.$('.powerState').on('click', this.onTap);
                this.isRendered = true;
            }
        }
    });

    PowerButton.MODE = {
        MODE_OFF: 0,
        MODE_ON: 1,
        MODE_SEARCHING: 2
    };

    return PowerButton;

});

define('widgets/wemo_lighting/js/tile',['backbone', 'hbs!widgets/wemo_lighting/templates/tile', 'extensions/ui/powerButton'],
    /** @exports tile view */
    function(Backbone, template, PowerButton) {

        'use strict';

        //will store the settings for when we start cycling the power button.

        var View = Backbone.TileBaseView.extend({
            template: template,
            isBeingSaved: false,

            /**
             * adds event binding to elements, clicks are converted to pointerup events
             */
            events: {
                'click .description': 'setToggle'
            },
            /**
             * two-way data binding to models, uses Backbone.stickit
             */
            bindings: {
                '*[data-bind="friendlyName"]': {
                    observe: 'friendlyName',
                    updateMethod: 'html'
                },
                '*[data-bind="LevelControl"]': {
                    observe: 'levelControl',
                    onGet: function(val) {
                        return this.model.getLevelControl() + '%';
                    }
                }
            },
            /**
             * Binds DOM events
             */
            render: function() {
                this.delegateEvents();
                this.renderPowerButton();
                this.changeButtonUI();
            },
            renderPowerButton: function(){
                this.powerButton = new PowerButton({
                    $el: this.$('.power'),
                    onTap: this.setMode.bind(this)
                });
                this.powerButton.render();
            },
            /**
             * Called when power button is clicked
             */
            setMode: function() {
                var state = parseInt(this.model.get('onOff'), 10),
                    newState;

                if (!this.isBeingSaved) {
                    this.powerButton.setState(PowerButton.MODE.MODE_SEARCHING);

                    if (state === 0) {
                        newState = 1;
                    } else {
                        newState = 0;
                    }

                    if (!this.$('.drawerArrow').hasClass('open')) { //when full drawer open, clicking power button will not change drawer
                        if ((newState === 1) || this.$('.description').hasClass('open')) {
                            var self = this;
                            setTimeout(function(){
                                self.peekaboo.toggle();
                                self.accordion.toggle(self.$('description'), true);
                            }, 500);
                        }
                    }

                    this.isBeingSaved = true;
                    this.model.set('onOff', newState);
                    this.sandbox.emit("onOffChanged");
                    this.model.saveToCache();
                    var sleepFader = this.model.get('sleepFader').split(/:(.+)?/)[0];
                    var attrsToSave;
                    if (newState === 0 && sleepFader > 0) {
                        this.model.set('sleepFader', "0:0");
                        attrsToSave = ['onOff', 'sleepFader'];
                    } else {
                        attrsToSave = newState === 0 ? ['onOff'] : ['onOff', 'levelControl'];
                    }
                    this.model.saveAction(attrsToSave).done(this.successSaveAction).fail(this.failSave);
                }
            },
            /**
             * Logic to change tue UI of the power button
             */
            changeButtonUI: function() {
                var mode = parseInt(this.model.get('onOff'), 10);

                if (!this.isBeingSaved) {
                    switch(mode){
                        case 0:
                            this.powerButton.setState(PowerButton.MODE.MODE_OFF);
                            break;
                        case 1:
                            this.powerButton.setState(PowerButton.MODE.MODE_ON);
                            break;
                        default:
                            this.powerButton.setState(PowerButton.MODE.MODE_OFF);
                            break;
                    }
                }
            },
            /**
             * put non-DOM event listeners here. These need to be removed when no longer necessary to free up memory (this.stopListening())
             */
            listen: function() {
                this.listenTo(this.model, 'change:onOff', this.changeButtonUI);
                this.listenTo(this.model, 'change:closeDrawer', this.closePeekaboo);
                this.listenTo(this.model, 'change:onOff', this.updateTimer);
                this.listenTo(this.model, 'change:levelControl', this.updateTimer);
                this.sandbox.on('close.peekaboo', this.closePeekaboo);
                this.sandbox.on('global.bridgeNotDiscovered' + this.model.baseObject.information.bridgeUDN, this.showUnavailable);
            },
            /**
             * Show greyout when link is not discovered
             */
            showUnavailable: function() {
                this.model.set('isDiscovered', false);
            },
            /**
             * Change peekaboo drawer timeout when user starting to change fan mode
             */
            updateTimer: function() {
                this.peekaboo.updateTimer();
            },
            /**
             * Function that close the peek a boo drawer
             */
            closePeekaboo: function() {
                this.accordion.close(true);
            },
            /**
             * Set behaviour for open, close drawer
             */
            setToggle: function(event) {
                this.accordion.toggle(event, false);
            },
            /**
             * Logic to set up the button to show/hide drawer cross-platform
             */
            afterInitialize: function() {
                var element = this.$('.noVisble');
                this.sandbox.uiInterface.setShowHideDrawer.call(this, element);
            },
            /**
             * When saveAction is success
             */
            successSaveAction: function(resp) {
                DEBUG && console.log("SUCCESS SAVE FOR LED:" + resp);
            },
            /**
             * Function that hide loading spinner after save action
             */
            afterSave: function() {
                this.isBeingSaved = false;
                this.changeButtonUI();
            },
            setDefaultFriendlyName: function() {
                this.model.set('defaultFriendlyName', this.i18n('defaultFriendlyName.' + this.model.baseObject.information.modelCode));
            },
            setDefaultIcon: function() {
                var modelCode = this.model.baseObject.information.modelCode.replace(/ /g, '_').toLowerCase();

                    var imageName = 'bulb_' + modelCode;
                this.$('.icon').addClass(imageName);
            }
        });

        return View;
    });

/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/summary',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  var buffer = "", stack1, options, helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;


  buffer += "<!--color still need this dependency-->\n<link rel=\"import\" href=\"components/polymer/components/paper-slider/paper-slider.html\">\n<div class=\"drawerContent\">\n    <section class=\"content\">\n        <div class=\"sliderContainer\">\n            <div class=\"slider\"></div>\n        </div>\n        <div class=\"colorPicker\" style=\"display: none\">\n            <div class=\"tempPicker\" data-bind=\"tempPicker\"></div>\n            <span class=\"colorTempTitle\" data-bind=\"colorTemp\"></span>\n\n            <span class=\"fa fa-angle-right\"></span>\n        </div>\n\n        <div class=\"sleepFader\">\n            <span>";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.sleepFader", options) : helperMissing.call(depth0, "i18n", "labels.sleepFader", options)))
    + "</span>\n            <span data-bind=\"timer\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "mode.off", options) : helperMissing.call(depth0, "i18n", "mode.off", options)))
    + "</span>\n        </div>\n        <div class=\"sleepOptions\">\n            <div class=\"parent\" data-value=\"1\">\n                <span class=\"information\">1m</span>\n                <span class=\"radBtn active\"></span>\n            </div>\n            <div class=\"parent\" data-value=\"10\">\n                <span class=\"information\">10m</span>\n                <span class=\"radBtn\"></span>\n            </div>\n            <div class=\"parent\" data-value=\"30\">\n                <span class=\"information\">30m</span>\n                <span class=\"radBtn\"></span>\n            </div>\n            <div id=\"custom\">\n                <span class=\"information\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.customOption", options) : helperMissing.call(depth0, "i18n", "labels.customOption", options)))
    + "</span>\n\n                <div class=\"form-list-select\" data-picker-type=\"mintimer\" data-picker-key=\"customOption\">\n                    <span class=\"button\" data-bind=\"customOption\"></span>\n                    <!--<select style=\"display:none\" name=\"timer\" class=\"widgetPicker timerOn\" data-bind=\"time\"></select>-->\n                </div>\n\n            </div>\n        </div>\n        <div id=\"sleepButton\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.sleep", options) : helperMissing.call(depth0, "i18n", "labels.sleep", options)))
    + "</div>\n    </section>\n</div>\n";
  return buffer;
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_summary', t);
return t;
});
/* END_TEMPLATE */
;
define('mixins/led.colorConversion',[], function() {
    'use strict';
    var COLOR_MAX_VALUE = 65536,
        RGB_MAX_VALUE = 255,
        DEFAULT_RGB_VALUE = '#ffffff';

    /**
     * converts temperature(in Kelvin) value to RGB value
     * @param tempNum {int} temperature value in Kelvin
     * @returns {string} rgb color
     */
    function convertTemperatureToRGB(tempNum) {
        var rgb = {};
        if (tempNum <= 2850) {
            rgb.r = 255;
            rgb.g = 181;
            rgb.b = 0;
        } else if (tempNum >= 6250) {
            rgb.r = 117;
            rgb.g = 142;
            rgb.b = 255;
        } else if (tempNum === 4000) {
            rgb.r = 255;
            rgb.g = 255;
            rgb.b = 255;
        } else {
            rgb.r = 255;
            rgb.g = 255;
            rgb.b = 255;

            // Convert value for red pixel color
            if (tempNum <= 4050) {
                rgb.r = 255;
            } else if (tempNum === 4600) {
                rgb.r = 231;
            } else if (tempNum === 5000) {
                rgb.r = 199;
            } else {
                var kvBase,
                    kvVal,
                    kvPercent,
                    redUpperLimit,
                    redLowerLimit;
                if (tempNum > 4050 && tempNum < 4600) {
                    kvBase = 4600 - 4050;
                    kvVal = tempNum - 4050;
                    kvPercent = Math.abs((kvVal / kvBase) - 1.0); // Inverted percentage
                    redUpperLimit = 255;
                    redLowerLimit = 231;
                } else if (tempNum > 4600 && tempNum < 5000) {
                    kvBase = (5000 - 4600);
                    kvVal = (tempNum - 4600);
                    kvPercent = Math.abs((kvVal / kvBase) - 1.0); // Inverted percentage
                    redUpperLimit = 231;
                    redLowerLimit = 199;
                } else if (tempNum > 5000 && tempNum < 6250) {
                    kvBase = (6250 - 5000);
                    kvVal = (tempNum - 5000);
                    kvPercent = Math.abs((kvVal / kvBase) - 1.0); // Inverted percentage
                    redUpperLimit = 199;
                    redLowerLimit = 117;
                }

                rgb.r = Math.round((kvPercent * (redUpperLimit - redLowerLimit)) + redLowerLimit);
            }

            // Convert value for green pixel color
            if (tempNum >= 3650 && tempNum <= 4550) {
                rgb.g = 255;
            } else if (tempNum === 5050) {
                rgb.g = 230;
            } else {
                var kvBase,
                    kvVal,
                    kvPercent,
                    greenUpperLimit,
                    greenLowerLimit;
                if (tempNum >= 2900 && tempNum < 3650) {
                    kvBase = (3600 - 2900);
                    kvVal = (tempNum - 2900);
                    kvPercent = (kvVal / kvBase);
                    greenUpperLimit = 255;
                    greenLowerLimit = 181;
                } else if (tempNum > 4550 && tempNum < 5050) {
                    kvBase = (5100 - 4550);
                    kvVal = (tempNum - 4550);
                    kvPercent = Math.abs((kvVal / kvBase) - 1.0); // Inverted percentage
                    greenUpperLimit = 255;
                    greenLowerLimit = 230;
                } else if (tempNum > 5050 && tempNum < 6250) {
                    kvBase = (6200 - 5050);
                    kvVal = (tempNum - 5050);
                    kvPercent = Math.abs((kvVal / kvBase) - 1.0); // Inverted percentage
                    greenUpperLimit = 230;
                    greenLowerLimit = 142;
                }
                rgb.g = Math.round((kvPercent * (greenUpperLimit - greenLowerLimit)) + greenLowerLimit);
            }

            // Convert value for blue pixel color
            if (tempNum > 4000) {
                rgb.b = 255;
            } else if (tempNum === 2900) {
                rgb.b = 2;
            } else if (tempNum === 2950) {
                rgb.b = 11;
            } else if (tempNum === 3600) {
                rgb.b = 142;
            } else if (tempNum === 3800) {
                rgb.b = 229;
            } else if (tempNum === 3850) {
                rgb.b = 235;
            } else if (tempNum === 3900) {
                rgb.b = 240;
            } else if (tempNum === 3950) {
                rgb.b = 245;
            } else {
                var kvBase,
                    kvVal,
                    kvPercent,
                    blueUpperLimit,
                    blueLowerLimit;
                if (tempNum > 2950 && tempNum < 3600) {
                    kvBase = (3550 - 2950);
                    kvVal = tempNum - 2950;
                    kvPercent = kvVal / kvBase;
                    blueUpperLimit = 142;
                    blueLowerLimit = 11;
                } else if (tempNum > 3600 && tempNum < 3800) {
                    kvBase = (3750 - 3600);
                    kvVal = tempNum - 3600;
                    kvPercent = kvVal / kvBase;
                    blueUpperLimit = 229;
                    blueLowerLimit = 142;
                }
                rgb.b = Math.round((kvPercent * (blueUpperLimit - blueLowerLimit)) + blueLowerLimit);
            }

        }

        return "rgb(" + rgb.r + "," + rgb.g + "," + rgb.b + ")";
    }

    /**
     * Converts HEX value to Color X,Y format for FW
     * @param hexinp {string}
     * @returns {string}
     */
    function HexToColor(hexinp) {
        if (hexinp) {
            var hex = parseInt(((hexinp.indexOf('#') > -1) ? hexinp.substring(1) : hexinp), 16),
                r = hex >> 16,
                g = (hex & 0x00FF00) >> 8,
                b = (hex & 0x0000FF),
                X = 0.584150 * r + 0.220650 * g + 0.116630 * b,
                Y = 0.228580 * r + 0.743650 * g + 0.027770 * b,
                Z = 0.019500 * r + 0.106800 * g + 0.583500 * b,
                sum = X + Y + Z;
            X = X / sum;
            Y = Y / sum;
            X = 65536 * X;
            Y = 65536 * Y;

            return Math.round(X) + ":" + Math.round(Y);
        }

    }

    /**
     * Converts COLOR X,Y value from FW to Hex
     * @param ColorX {number} Color space X coordinate denominator
     * @param ColorY {number} Color space Y coordinate denominator
     * @returns {string} Hexcode
     */
    function convertColorToHex(ColorX, ColorY) {
        var returnValue;
        if (ColorX && ColorY && ColorX !== 0 && ColorY !== 0) {
            ColorX = Math.round(ColorX) / COLOR_MAX_VALUE;
            ColorY = Math.round(ColorY) / COLOR_MAX_VALUE;
            var z = parseFloat(1 - ColorX - ColorY),
                Y = 1.0,
                X = ColorX * (Y / ColorY);
            z = z * (Y / ColorY);

            var R = (1.927450 * X) + (-0.520123 * Y) + (-0.360505 * z),
                G = ((-0.594107 * X) + (1.514293 * Y) + (0.046682 * z)),
                B = ((0.044328 * X) + (-0.259784 * Y) + (1.717300 * z)),
                scalar = Math.max(R, Math.max(G, B));

            R = R / scalar;
            G = G / scalar;
            B = B / scalar;

            R = R.toFixed(2);
            G = G.toFixed(2);
            B = B.toFixed(2);

            R = Math.round(R * RGB_MAX_VALUE);
            B = Math.round(B * RGB_MAX_VALUE);
            G = Math.round(G * RGB_MAX_VALUE);

            if (R > RGB_MAX_VALUE) {
                R = RGB_MAX_VALUE;
            } else if (B > RGB_MAX_VALUE) {
                B = RGB_MAX_VALUE;
            } else if (G > RGB_MAX_VALUE) {
                G = RGB_MAX_VALUE;
            }


            R = Math.abs(R);
            B = Math.abs(B);
            G = Math.abs(G);

            var hexVal = '#' + byteToHex(R) + byteToHex(G) + byteToHex(B);
            returnValue = hexVal;
        } else {
            returnValue = DEFAULT_RGB_VALUE;
        }
        return returnValue;

    }

    /**
     * Converts RGB bytes to Hexcode
     * @param n {number} RGB byte
     * @returns {string}
     */
    function byteToHex(n) {
        var nybHexString = '0123456789ABCDEF';
        return String(nybHexString.substr((n >> 4) & 0x0F, 1)) + nybHexString.substr(n & 0x0F, 1);
    }

    var exports = {
        convertTemperatureToRGB: convertTemperatureToRGB,
        HexToColor: HexToColor,
        convertColorToHex: convertColorToHex,
        byteToHex: byteToHex

    };


    return exports;

});

define('libs/range',['jquery'], function($){
    return function Range() {
        this._updateSliderFill = function(){
            var $target = this.$el.find('input[type=range]');
            var width = ($target.parent().width() - $target.parent().height()) * ($target.val()/$target.attr('max'));
            $target.parent().find('.fill').css('width', width + 'px');
        };

        this.init = function(el, options){
            console.log("Initing");

            if(!el){
                return console.error('You must specify an element to render the range slider!');
            }
            _.defaults(options, {
                size: 24,
                min: 0,
                max: 100,
                value: 0,
                thumbColor: '#73D44C',
                fillColor: '#ADE48C',
                onChange: function(){},
                onTouchMove: function(){}
            });

            var id = 'range-' + (new Date()).getTime();
            var $el = $(el);
            $el.attr('id', id);
            this.$el = $el;

            // Add styles
            var styles = [];
            styles.push(".slider{position: relative;z-index: 0;}");
            styles.push(".slider input[type=range] {-webkit-appearance: none;display: block;position: relative;margin: 0;width: 100%;");
            styles.push(".slider input[type=range]:focus {outline: none;}");
            styles.push(".slider input[type=range]::-webkit-slider-runnable-track {width: 100%;cursor: pointer;box-shadow: none;background: #f2f2f2;border: 0;}");
            styles.push(".slider input[type=range]::-webkit-slider-runnable-track {border-radius: " + options.size/2 + "px;height: " + options.size + "px;}");
            styles.push(".slider input[type=range]::-webkit-slider-thumb {box-shadow: inset 0 -1px 0 0 rgba(0,0,0,0.25), inset 0 1px 0 0 rgba(255,255,255,.5);border: 0;cursor: pointer;-webkit-appearance: none;margin-top: 0px;z-index:1;position: relative;}");
            styles.push(".slider input[type=range]::-webkit-slider-thumb {height: " + options.size + "px;width: " + options.size + "px;border-radius: " + options.size/2 + "px;}");
            styles.push(".slider .fill {display: block;position: absolute;top: 0;z-index: 0;pointer-events: none}");
            styles.push(".slider .fill:before {content: '';display: block;position: absolute;top: 0;background:inherit;}");
            styles.push(".slider .fill:before {width: " + options.size + "px;height: " + options.size + "px;border-radius: " + options.size/2 + "px;left: -" + options.size/2 + "px;}");
            styles.push(".slider .fill {left: " + options.size/2 + "px;height: " + options.size + "px;}");

            _.forEach(styles, function(style){
                document.styleSheets[0].insertRule("#" + id + style, document.styleSheets[0].rules.length);
            });
            this.setThumbColor(options.thumbColor, $el);
            this.setFillColor(options.fillColor, $el);

            $el.empty();
            $el.append('<input type="range" min="' +options.min + '" max="' + options.max + '" step="1" value="' + options.value + '"/>');
            $el.append('<div class="fill"/>');

            var self = this;
            $el.find('input[type=range]').on('input change', function(e){
                self._updateSliderFill();
                options.onTouchMove(e);
            });

            $el.find('input[type=range]').on('output change', function(e){
                options.onChange(e);
            });

            setTimeout(function(){
                self._updateSliderFill();
            }, 100);
        };

        this.setValue = function(value){
            this.$el.find('input[type=range]').val(value);
            this._updateSliderFill();
        };

        this.setThumbColor = function(color, $el){
            if(!$el) return;
            var style = ".slider input[type=range]::-webkit-slider-thumb {background-color: "+ color +";}";
            document.styleSheets[0].insertRule("#" + $el.attr("id") + style, 1);

            if (color === "#3e4978") {
                $el.find('input[type=range]').addClass("nightModeSlider");
            } else {
                $el.find('input[type=range]').removeClass("nightModeSlider");
            }
        };

        this.setFillColor = function(color, $el){
            if(!$el) return;
            $el.find(".fill").css("background-color", color);
            var style = ".slider .fill {background-color:"+ color +";}"
            document.styleSheets[0].insertRule( "#" + $el.attr("id")+ style, 1);
        };
    }
});

define('widgets/wemo_lighting/js/summary',['Cache', 'hbs!widgets/wemo_lighting/templates/summary', 'mixins/led.colorConversion', 'libs/range'],
    /** @exports summary view */
    function(Backbone, template, colorConversion, Range) {
        'use strict';

        var View = Backbone.DrawerBaseView.extend({
            template: template,

            events: {
                'click .parent': 'selectTimer',
                'click .button': 'openSelect',
                'click #sleepButton': 'setTimer'
            },
            /**
             * two-way data binding
             */
            bindings: {
                '*[data-bind="customOption"]': {
                    observe: 'customOption',
                    onGet: function(val) {
                        var lbl = '',
                            $button = this.$('.button'),
                            state = this.model.get('onOff'),
                            time = parseInt(val, 10);

                        if (isNaN(time) || val === '') {
                            lbl = this.i18n('labels', 'btn');
                            $button.removeClass('customSet');
                        } else {
                            lbl = time + ' M';
                            $button.addClass('customSet');
                            this.$('.sleepOptions span').removeClass('active');
                            this.model.set('selectedTimer', time);
                        }
                        return lbl;
                    }
                },
                '*[data-bind="colorTemp"]': {
                    observe: 'pickerView',
                    onGet: function(val) {
                        var $picker = this.$('.colorPicker'),
                            lbl = '';
                        if (val !== 0) {
                            $picker.css("display", "flex");
                            switch (val) {
                                case 1:
                                    lbl = this.i18n("labels", "color") + ' ' + this.i18n('labels', 'temperature');
                                    break;
                                case 2:
                                    lbl = this.i18n("labels", "color");
                                    break;
                                case 3:
                                    lbl = this.i18n("labels", "color") + '/' + this.i18n('labels', 'temperature');
                                    break;
                            }
                            return lbl;
                        }
                    }
                },
                '*[data-bind="tempPicker"]' : {
                    observe: ['colorControl', 'colorTemperature'],
                    update: function(view, values) {
                        this.updateTempColor();
                    }
                }
            },
            /**
             * localizes the template, and inserts it into the document fragment. Also binds the DOM events
             */
            render: function() {
                this.delegateEvents();
                this.setDefaultUI();
                this.runTimer();
                this.checkProductType();
                this.renderSleepButton();
                this.sandbox.uiInterface.showSettings.call(this, this.$('.colorPicker'));

                var self = this;
                this.brightnessRange = new Range();
                setTimeout(function(){
                    self.brightnessRange.init(self.$('.slider'), {
                        thumbColor: '#73D44C',
                        fillColor: '#ADE48C',
                        size: 28,
                        min: 1,
                        value: self.model.getLevelControl(),
                        onTouchMove: self.changeDimValue.bind(self),
                        onChange: self.setState.bind(self)
                    });
                }, 0);



            },
            /**
             * Opens the custom time selector
             * @param {DOM element} event DOM element that was clicked on
             */
            openSelect: function(event) {
                var $target = $(event.currentTarget),
                    self = this;

                $target.mobiscroll().timespan({
                        wheelOrder: 'i',
                        theme: 'wemoModal customMinTimer',
                        minTime: 60000,
                        maxTime: 3600000,
                        display: 'modal',
                        rows: 3,
                        minWidth:210,
                        headerText: function(){
                            return self.i18n('labels', 'custom');
                        },
                        labels: ['Y', 'M', 'D', 'H', 'M', 'S'],
                        buttons: [
                            'cancel',
                            'set'
                        ],
                        closeOnOverlay: false,
                        onBeforeShow: function() {
                            $target.mobiscroll('setValue', self.model.get('customOption'));
                        },
                        onClose: function(value, btn) {
                            if (btn === 'set') {
                                var m = parseInt(value.split(' ')[0]);
                                self.model.set('customOption', m);
                            }
                        }
                });
            },
            /**
             *
             */
            checkProductType: function() {
                var device = this.model.baseObject.information.productName,
                    pickerView = this.model.productNameObj[device] ? this.model.productNameObj[device] : 0;
                this.model.set('pickerView', pickerView);
                this.updateTempColor();
            },
            /**
             * put non-DOM event listeners here. These need to be removed when no longer necessary to free up memory (this.stopListening())
             */
            listen: function() {
                this.listenTo(this.model, 'change:onOff', this.setDefaultUI);
                this.listenTo(this.model, 'change:onOff', this.renderSleepButton);
                this.listenTo(this.model, 'change:sleepFader', this.runTimer);
                this.listenTo(this.model, 'change:levelControl', this.updateSlider);
                this.sandbox.on('drawer.open', this.updateDrawer);
                this.sandbox.on("tempColorChanged", this.updateTempColor);
                this.sandbox.on("onOffChanged", this.renderSleepButton);
            },
            updateDrawer: function() {
                this.isChangingFromSlider = false;
                this.updateSlider();
            },
            /**
             * Updates the slider with current value
             */
            updateSlider: function() {
                if (!this.isChangingFromSlider) {
                    var brightness =  this.model.getLevelControl();
                    this.brightnessRange.setValue(brightness);
                }
            },
            /**
             * Logic for sleep fader timer
             */
            runTimer: function() {
                var sleepFader = this.model.get('sleepFader');

                if (!sleepFader) {
                    sleepFader = '0:0';
                    this.model.set('sleepFader', sleepFader);
                }
                var sleepFaderSplit = sleepFader.split(':'),
                    transition = parseInt(sleepFaderSplit[0], 10) / 10,
                    startTime = parseInt(sleepFaderSplit[1], 10),
                    currentTime = Math.round(new Date().getTime() / 1000.0),
                    endTime = startTime + transition,
                    leftTime = endTime - currentTime,
                    $elem = this.$('*[data-bind="timer"]');
                if (endTime !== 0 && leftTime > 0) {
                    var minutes = ('0' + Math.floor((leftTime / 60))).slice(-2),
                        seconds = ('0' + (leftTime - (minutes * 60))).slice(-2),
                        lbl = '',
                        self = this;
                    if (minutes < 1) {
                        lbl = seconds + 's' + ' ' + this.i18n('labels', 'remaining');
                    } else {
                        lbl = minutes + ':' + seconds + ' ' + this.i18n('labels', 'remaining');
                    }
                    $elem.html(lbl);
                    if (!this.countDownInterval) {
                        this.countDownInterval = setInterval(function() {
                            self.runTimer();
                        }, 1000);
                    }
                } else {
                    if (this.countDownInterval) {
                        $elem.html(this.i18n("mode", "off"));
                        clearInterval(this.countDownInterval);
                        this.countDownInterval = false;
 						this.renderSleepButton();
                    }
                }
            },
            renderSleepButton : function() {
               var mode = parseInt(this.model.get('onOff'), 10);
                if (mode === 0)  {
                    this.$el.find('#sleepButton').addClass('disableSleepFader')
                        .css("color", "#ccc");
                } else {
                    this.$el.find('#sleepButton').removeClass('disableSleepFader')
                        .css("color", "#73D34A");
                }
            },
            /**
             * Logic to update color in the drawer UI
             */
            updateTempColor: function() {
                var pickerView = this.model.get('pickerView'),
                    activeCapability = this.model.get('activeCapability'),
                    device = this.model.baseObject.information.productName;
                if (pickerView !== 0) {
                    var temp = this.model.get('colorTemperature'),
                        color = this.model.get('colorControl'),
                        $picker = this.$('.tempPicker'),
                        valueToConvert = 0,
                        value = 'fff';

                    if (pickerView === 1 || activeCapability === 'colorTemperature') {
                        valueToConvert = Math.round(1000000 / temp.split(':')[0]);
                        value = colorConversion.convertTemperatureToRGB(valueToConvert);
                    } else {
                        valueToConvert = color.split(':');
                        value = colorConversion.convertColorToHex(valueToConvert[0], valueToConvert[1]);
                    }
                    $picker.css('background-color', value);
                }
            },
            /**
             * sets default UI when on/off
             */
            setDefaultUI: function() {
                var state = parseInt(this.model.get('onOff'), 10),
                    timer = parseInt(this.model.get('sleepFader').split(':')[0], 10) / 600;
                //show/hide sleep button
                if (state === 0) {
                    if (this.sleepInterval) {
                        this.model.set('timer', 0);
                        this.clearTimer();
                    }
                } 
                //show the corresponding sleep fader value
                if (timer === 1 || timer === 10 || timer === 30) {
                    this.model.set('customOption', '');
                    this.$('.sleepOptions span').removeClass('active');
                    this.$(".parent[data-value='" + timer + "'] .radBtn").addClass("active");
                } else if (timer !== 0) {
                    this.model.set('customOption', timer);
                }
            },
            /**
             * Logic to update the button UI when custom option is selected
             * @param {DOM element} e DOM element that was clicked on
             */
            selectTimer: function(e) {
                var $opt = this.$(e.currentTarget),
                    timerValue = $opt.attr('data-value');
                this.$('.sleepOptions span').removeClass('active');
                this.$('.button').removeClass('customSet');
                $opt.find('.radBtn').addClass('active');
                this.model.set('selectedTimer', timerValue);
            },
            /**
             * sets the sleep fader timer
             */
            setTimer: function() {
                var timer = (parseInt(this.model.get('selectedTimer'), 10) * 600),
                    state = parseInt(this.model.get('onOff'), 10),
                    currEpoch = Math.round(new Date().getTime() / 1000.0);
                if (state === 1) {
                    this.model.set('sleepFader', timer + ':' + currEpoch);
                    this.model.saveAction(['sleepFader']).done(this.successSaveAction).fail(this.failSaveAction);
                }
            },
            /**
             * clears the timer when necessary
             */
            clearTimer: function() {
                clearInterval(this.sleepInterval);
                this.sleepInterval = false;
            },
            /**
             * changes the value of the slider
             * @param {DOM element} e DOM element that represents current value of the slider
             */
            changeDimValue: function(e) {
                var value = e.target.value;
                if (this.model.getLevelControl() !== value) {
                    this.model.setLevelControl(value);
                    this.isChangingFromSlider = true;
                }

            },
            /**
             * Logic to change fader value
             */
            setState: function() {
                var sleepFader = this.model.get('sleepFader').split(/:(.+)?/)[0];
                //when sleepfader is on and dim  val is changes then set sleepfader to 0:0
                if (sleepFader > 0) {
                    this.model.set('sleepFader', '0:0');
                    this.model.saveAction(['levelControl', 'sleepFader']).done(this.successSaveAction).fail(this.failSaveAction);
                } else {
                    var mode = parseInt(this.model.get('onOff'));
                    if (mode == 0) {
                        //set device on only when device is off and user change state
                        this.model.set('onOff', '1');
                        this.model.saveAction(['onOff', 'levelControl']).done(this.successSaveAction).fail(this.failSaveAction);
                    } else {
                        this.model.saveAction(['levelControl']).done(this.successSaveAction).fail(this.failSaveAction);
                    }
                }
                this.isChangingFromSlider = false;
            },
            /**
             * When saveAction is success
             */
            successSaveAction: function(resp) {
                DEBUG && console.log('LED:SUCCESS:SAVE:ACTION');
            },
            /**
             * When saveAction fails
             */
            failSaveAction: function() {
                DEBUG && console.log('LED:ERROR:SAVE:ACTION');
            },
            /**
             * Logic when drawer is closed
             */
            onClose: function() {
                if (this.countDownInterval) {
                    clearInterval(this.countDownInterval);
                    this.countDownInterval = false;
                }
            }

        });

        return View;
    });

/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/colorPicker',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  var buffer = "", stack1, options, helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;


  buffer += "<link rel=\"import\" href=\"components/polymer/components/wemo/color-picker/color-picker.html\">\n<header class=\"showForAndroid\">\n    <div class=\"back\">\n        <span class=\"backIcon\"></span>\n        <span class=\"title\" data-bind=\"colorTemp\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.color", options) : helperMissing.call(depth0, "i18n", "labels.color", options)))
    + "</span>\n    </div>\n</header>\n<section class=\"content\">\n    <color-picker id=\"colorPicker\"></color-picker>\n</section>\n";
  return buffer;
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_colorPicker', t);
return t;
});
/* END_TEMPLATE */
;
/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/temperaturePicker',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  var buffer = "", stack1, options, helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;


  buffer += "<link rel=\"import\" href=\"components/polymer/components/wemo/temperature-picker/temperature-picker.html\">\n<header class=\"showForAndroid\">\n    <div class=\"back\">\n        <span class=\"backIcon\"></span>\n        <span class=\"title\"  data-bind=\"colorTemp\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.color", options) : helperMissing.call(depth0, "i18n", "labels.color", options)))
    + " ";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.temperature", options) : helperMissing.call(depth0, "i18n", "labels.temperature", options)))
    + "</span>\n    </div>\n</header>\n<section class=\"content\">\n    <temperature-picker id='temperaturePicker'></temperature-picker>\n</section>\n";
  return buffer;
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_temperaturePicker', t);
return t;
});
/* END_TEMPLATE */
;
/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/colorTemperaturePickers',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  var buffer = "", stack1, options, helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;


  buffer += "<link rel=\"import\" href=\"components/polymer/components/wemo/color-picker/color-picker.html\">\n<link rel=\"import\" href=\"components/polymer/components/wemo/temperature-picker/temperature-picker.html\">\n<header class=\"specialCase showForAndroid\">\n    <div class=\"back\">\n        <span class=\"backIcon\"></span>\n        <span class=\"title\"  data-bind=\"colorTemp\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.color", options) : helperMissing.call(depth0, "i18n", "labels.color", options)))
    + "/";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.temperature", options) : helperMissing.call(depth0, "i18n", "labels.temperature", options)))
    + "</span>\n    </div>\n</header>\n<section class=\"content\">\n    <div class=\"tabs\">\n        <span id=\"colorTab\" class=\"active\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.color", options) : helperMissing.call(depth0, "i18n", "labels.color", options)))
    + "</span>\n        <span id=\"tempTab\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.temperature", options) : helperMissing.call(depth0, "i18n", "labels.temperature", options)))
    + "</span>\n    </div>\n    <color-picker id=\"colorPicker\"></color-picker>\n    <temperature-picker id='temperaturePicker' style=\"display:none\"></temperature-picker>\n</section>\n";
  return buffer;
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_colorTemperaturePickers', t);
return t;
});
/* END_TEMPLATE */
;
define('widgets/wemo_lighting/js/settings',['Cache', 'hbs!widgets/wemo_lighting/templates/colorPicker', 'hbs!widgets/wemo_lighting/templates/temperaturePicker', 'hbs!widgets/wemo_lighting/templates/colorTemperaturePickers', 'mixins/led.colorConversion'],
    /** @exports settings view */
    function(Backbone, colorPicker, temperaturePicker, colorTemperaturePickers, colorConversion) {
        'use strict';

        var View = Backbone.OverlayView.extend({
            tagName: 'div',
            viewName: 'settings noMainHeader noScroll',
            template1: temperaturePicker,
            template2: colorPicker,
            template3: colorTemperaturePickers,
            hasManualTimerChange: false,
            capabilityToSet: 'colorTemperature',
            isChangingFromPicker: true,

            /**
             * adds event binding to elements, clicks are converted to pointerup events
             */
            events: {
                'tap header': 'goBack',
                'drag-end #colorPicker': 'changeColor',
                'drag-end #temperaturePicker': 'changeColor',
                'prefix-selected #colorPicker': 'changeColor',
                'prefix-selected #temperaturePicker': 'changeColor',
                'tap .tabs span': 'changePickerView'
            },
            /**
             * two-way data binding to models, uses Backbone.stickit
             */
            bindings: {},
            /**
             * Checks for productName to load the corresponding template
             */
            afterInitialize: function() {
                var pickerView = this.model.get('pickerView');
                this.capabilityToSet = (pickerView !== 1) ? 'colorControl' : 'colorTemperature';
                this.template = this['template' + pickerView];
            },
            /**
             * Logic to show the corresponding picker(color/temperature)
             * @param $tab {JQuery element}
             */
            changePickerView: function($tab) {
                var $curr = this.$($tab.currentTarget),
                    $colorPicker = this.$('#colorPicker'),
                    $temperaturePicker = this.$('#temperaturePicker');
                this.$('.tabs span').removeClass('active');
                this.capabilityToSet = $curr.attr('id') === 'colorTab' ? 'colorControl' : 'colorTemperature';
                if (this.capabilityToSet === 'colorControl') {
                    $temperaturePicker.hide();
                    $colorPicker.show();
                } else {
                    $colorPicker.hide();
                    $temperaturePicker.show();
                }
                $curr.addClass('active');
            },
            /**
             * Sets the color/temperature to the device
             * @param $el {JQuery element}
             */
            changeColor: function($el) {
                    var pickerValue = $el.currentTarget.id === 'colorPicker' ? $el.target.selectedColor : $el.target.selectedTemperature,
                        value = 0,
                        valuesToSet = {
                            activeCapability : this.capabilityToSet
                        };
                    if (pickerValue) {
                        if (this.capabilityToSet === 'colorTemperature') {
                            value = Math.round(1000000 / pickerValue);
                        } else {
                            value = colorConversion.HexToColor(pickerValue);
                        }
                        valuesToSet[this.capabilityToSet] = value + ':0';

                        this.model.set(valuesToSet);
                        this.sandbox.emit("tempColorChanged");

                        var sleepFader =this.model.get('sleepFader').split(/:(.+)?/)[0];

                        //when sleepfader is on and picker value is greater then 0 then set sleepfader to 0:0
                        if(sleepFader>0){
                            this.model.set('sleepFader', "0:0");
                            this.model.saveAction([this.capabilityToSet,'sleepFader']).done(this.checkForOn).fail(this.failSaveAction);
                         }else{
                             var mode = parseInt(this.model.get('onOff'));
                             if(mode==0){
                                this.model.set('onOff', "1");
                                this.model.saveAction(['onOff',this.capabilityToSet]).done(this.checkForOn).fail(this.failSaveAction);
                             }else{
                                 this.model.saveAction([this.capabilityToSet]).done(this.checkForOn).fail(this.failSaveAction);
                             }
                        }
                    }
            },
            /**
             * localizes the template, and inserts it into the document fragment
             */
            render: function() {
                //this.$el.addClass('settings');
                this.model = this.sandbox.model;
                this.setPickerValue();
            },
            /**
             * Sets the picker value based on productName
             */
            setPickerValue: function() {
                var temp = this.model.get('colorTemperature'),
                    color = this.model.get('colorControl'),
                    pickerView = this.model.get('pickerView'),
                    activeCapability = this.model.get('activeCapability'),
                    $tempPicker = this.$('#temperaturePicker'),
                    $colorPicker = this.$('#colorPicker'),
                    valueToConvert = 0,
                    value = 0,
                    round = 50;
                if (pickerView === 1 || activeCapability === 'colorTemperature') {
                    value = 1000000 / temp.split(':')[0];
                    value = value / round;
                    value = Math.round(value) * round;
                    if (pickerView === 3) {
                        $tempPicker.show();
                        $colorPicker.hide();
                        this.$('#colorTab').removeClass('active');
                        this.$('#tempTab').addClass('active');
                    }
                    $tempPicker.attr('temperature', value);

                } else {
                    valueToConvert = color.split(':');
                    value = colorConversion.convertColorToHex(valueToConvert[0], valueToConvert[1]);
                    $colorPicker.attr('color', value);
                }
            },
            /**
             * Logic to turn on the LED when corresponding
             */
            checkForOn: function() {
                var onOff = parseInt(this.model.get('onOff'), 10);
                if (onOff === 0) {
                    this.model.set('onOff', 1);
                    this.model.saveAction(['onOff']).done(this.successSaveAction).fail(this.failSaveAction);
                }
            },
            /**
             * When saveAction is success
             */
            successSaveAction: function(resp) {
                DEBUG && console.log('LED:SUCCESS:SAVE:ACTION');
            },
            /**
             * When saveAction fails
             */
            failSaveAction: function() {
                DEBUG && console.log('LED:ERROR:SAVE:ACTION');
            },
            /**
             * callback that occurs after the settings view is written to the screen
             */
            afterRender: function() {
                this.delegateEvents();
                var self = this;
            },
            /**
             * callback that occurs before the settings view opens
             */
            beforeOpen: function() {

            },
            /**
             * put non-DOM event listeners here. These need to be removed when no longer necessary to free up memory (this.stopListening())
             */
            listen: function() {

            },
            /**
             * Closes the view and goes back to drawer
             */
            goBack: function() {
                var self = this;
                self.sandbox.uiInterface.dismissSettings.call(self, {
                    cancel: true
                });
                this.stopConfirmDialog();
            }
        });

        return View;
    });

define('widgets/wemo_lighting/js/drawer',['Cache','widgets/wemo_lighting/js/summary','widgets/wemo_lighting/js/settings'],
/** @exports drawer */
function(Backbone,SummaryView,SettingsView){

    'use strict';

    var View = Backbone.DrawerMasterView.extend({
        /**
         * determines which view to show on initial load
         */
        render: function(){
            var view;
            /** Set the summary view, for now*/
            view = 'summary';
            this.changeView(view);
        },
        loadedViews : {
            summary:{
                view:SummaryView,
                viewType:'drawer'
            },
            settings: {
                view: SettingsView,
                viewType: 'overlay'
            }
        }
    });

    return View;
});
/* START_TEMPLATE */
define('hbs!widgets/wemo_lighting/templates/edit',['hbs','handlebars'], function( hbs, Handlebars ){ 
var t = Handlebars.template(function (Handlebars,depth0,helpers,partials,data) {
  this.compilerInfo = [3,'>= 1.0.0-rc.4'];
helpers = helpers || Handlebars.helpers;
  var buffer = "", stack1, options, helperMissing=helpers.helperMissing, escapeExpression=this.escapeExpression;


  buffer += "<div class=\"checkBulb_container\">\n    <span id=\"find_bulb\" class=\"find_bulb\">";
  options = {hash:{}};
  buffer += escapeExpression(((stack1 = helpers.i18n),stack1 ? stack1.call(depth0, "labels.checkBulb", options) : helperMissing.call(depth0, "i18n", "labels.checkBulb", options)))
    + "</span>\n</div>";
  return buffer;
  });
Handlebars.registerPartial('widgets_wemo_lighting_templates_edit', t);
return t;
});
/* END_TEMPLATE */
;
define('libs/execHandler',[], function() {
    var ExecHandler = function() {
        this.initialize.apply(this, arguments);
    };
    if (!window.smartDevicePlugin) {
        window.smartDevicePlugin = {};
    }

    window.smartDevicePlugin.callbackList = {};
    window.smartDevicePlugin.onDeviceChanged = function(id, resp) {
        //find the device, fire off the first callback in the array, then get rid of it.
        DEBUG && console.log('ExecHandler:onDeviceChanged:RESPONDED:' + id);


        var callback = window.smartDevicePlugin.callbackList[id];
        if (callback && id) {
            if (callback.length > 0) {
                var self = callback[0];
                clearTimeout(self.timeout);

                if (typeof resp == 'string') {
                    resp = JSON.parse(resp);
                }

                if (((self.action == 'setDeviceState' || self.action == 'setGroupState') && resp.onDeviceUpdated) || (!resp.onDeviceUpdated && resp.success === 'true') || (self.action == 'setDeviceProperties' && resp.success === 'true')) {
                    DEBUG && console.log('ExecHandler:useWindowMethod:SUCCESS:' + self.plugin + ':' + self.action + ':' + JSON.stringify(resp));
                    if (self.callback) {
                        self.callback(resp);
                    }
                    self.dfd.resolve(resp);
                } else {
                    DEBUG && console.log('ExecHandler:useWindowMethod:ERROR:' + self.plugin + ':' + self.action + ':' + JSON.stringify(resp));
                    self.dfd.reject(resp);
                }

                callback.splice(0, 1);
            }
        }
    }

    ExecHandler.prototype = {
        constructor: ExecHandler,
        initialize: function(options) {
            _.bindAll(this);
            options = options || {};
            _.extend(this, options);
            this.dfd = new $.Deferred();
            this.timeout = setTimeout(function() {}, 0);

        },
        success: function(resp) {
            if (!this.useWindowMethod) {
                DEBUG && console.log('ExecHandler:SUCCESS:' + this.plugin + ':' + this.action + ':' + JSON.stringify(resp));
                if (this.callback) {
                    this.callback(resp);
                }
                this.dfd.resolve(resp);
            }
        },
        initWindowMethod: function() {
            //create an array of callbacks to this device.

            if (!window.smartDevicePlugin.callbackList[this.useWindowMethod]) {
                window.smartDevicePlugin.callbackList[this.useWindowMethod] = [];
            }

            window.smartDevicePlugin.callbackList[this.useWindowMethod].push(this);

        },
        error: function(err) {
            if (!this.useWindowMethod) {
                DEBUG && console.log('ExecHandler:ERROR:' + this.plugin + ':' + this.action + ':' + JSON.stringify(err));
                //in cases where device is not reachable, err = {isDiscovered:false}, update this model to cause greyout
                if (this.errorCallback) {
                    this.errorCallback(err);
                }
                this.dfd.reject(err);
            }
        },
        go: function(plugin, action, data) {
            var self = this;
            data = data || [];
            this.plugin = plugin;
            this.action = action;
            DEBUG && console.log('ExecHandler:' + plugin + ':' + action + ':' + JSON.stringify(data));
            this.isRemote = (window.sessionStorage.getItem('isRemote') === "true");
            this.timeoutLimit = 15000;

            if (this.useWindowMethod) {
                this.initWindowMethod();
            }

            if (window.cordova) {
                DEBUG && console.log('ExecHandler:' + plugin + ':' + action + ':sending to Cordova');
                window.cordova.exec(this.success, this.error, plugin, action, data);
            } else {
                if (this.useWindowMethod) {

                    var success = 'true';

                    var jsonString = JSON.stringify({
                        udn: this.useWindowMethod,
                        success: success
                    });
                    window.smartDevicePlugin.onDeviceChanged(this.useWindowMethod, jsonString);
                } else {
                    this.success({});
                }
            }

            if (!this.ignoreTimeout) {
                this.timeout = setTimeout(function() {
                    window.smartDevicePlugin.onDeviceChanged(self.useWindowMethod, {
                        success: false,
                        error: 'timeout'
                    });
                }, self.timeoutLimit);
            }
            return this.dfd.promise();
        }
    }

    return ExecHandler;

});
/** a common API for devices and the browser. the browser version returns mock data for testing, will be used as part of an extension to mixin with the sandbox  **/
define('libs/smartDevicePlugin',['libs/execHandler', 'libs/constants'], function(ExecHandler, Constants) {
    'use strict';

    var callPlugin = function(plugin, action, data, options) {
        var handler = new ExecHandler(options);
        return handler.go(plugin, action, data);
    }

    var exports = {
        /**
         * sends device to Native layer to update state via UPNP or cloud
         * @param {function} callback a callback method to fire on success, if one is defined.
         * @param {JSON object} model [the entire device model]
         * @param {JSON object} data        [contains state attributes]
         * @return {promise}
         */
        setDeviceState: function(callback, model, data) {
            return callPlugin('SmartDevicePlugin', 'setDeviceState', [model.baseObject.information, data], {
                callback: callback,
                model: model,
                useWindowMethod: model.baseObject.information.udn
            });
        },
        setDeviceProperties: function(callback, model, data) {
            return callPlugin('SmartDevicePlugin', 'setDeviceProperties', [model.baseObject.information, data], {
                callback: callback,
                model: model,
                useWindowMethod: model.baseObject.information.udn
            });
        },
        setNightModeConfiguration: function(callback, model, data) {
            return callPlugin('SmartDevicePlugin', 'setNightModeConfiguration', [data], {
                callback: callback
            });
        },
        /**
         * updates the state for all devices in a group
         * @param {Backbone.Model} model the device model
         * @param {JSON} data  the attributes to save
         */
        setGroupState: function(model, data) {
            return callPlugin('SmartDevicePlugin', 'setGroupState', [model.baseObject.information, data], {
                model: model,
                callback: function() {
                    model.trigger('save');
                },
                useWindowMethod: model.baseObject.properties.groupID
            });
        },
        collectEmailId:function(){
            return callPlugin('SmartDevicePlugin', 'collectEmailID');
        },
        /**
         * gets all discovered devices from smartCache
         * @return {promise}
         */
        getDevices: function() {
            return callPlugin('SmartDevicePlugin', 'getDevices');
        },
        getDevicesInfo: function() {
            return callPlugin('SmartDevicePlugin', 'getDevicesInfo');
        },
        /**
         * gets a single device from smartCache (smartCache must get data from device)
         * @return {promise}
         */
        getDevice: function(callback, model, options) {
            return callPlugin('SmartDevicePlugin', 'getDevice', [model.baseObject.information], {
                callback: callback,
                model: model
            });
        },
        checkNestAuth: function () {
            return callPlugin('SmartDevicePlugin', 'nestTokenExistForGivenHomeId');
        },

        getNestDevices: function () {
            return callPlugin('SmartDevicePlugin', 'getNestDevices');
        },
        /**
         * gets updated insight values every 5 seconds from smartCache (smartCache must get data from device)
         * @return {promise}
         */
        getInsightParameters: function(callback, model, options) {
            return callPlugin('SmartDevicePlugin', 'getInsightParameters', [model.baseObject.information], {
                callback: callback,
                model: model
            });
        },
        setInsightDrawerState: function(deviceInfo) {
            return callPlugin('SmartDevicePlugin', 'setInsightDrawerState', [deviceInfo]);
        },
        /**
         * refreshes device list - triggers re-discovery in smartCache
         * @return {promise}
         */
        refreshDevices: function() {
            return callPlugin('SmartDevicePlugin', 'refreshDevices');
        },
        /**
         * adds a device to a group
         * @param  {string} udn     unique device id
         * @param  {string} groupId unique group id
         * @param {boolean} isNewGroup true when creating a new group
         * @return {promise}
         */
        addToGroup: function(bridgeUDN, payload, isNewGroup) {
            if (!isNewGroup) isNewGroup = false;
            return callPlugin('SmartDevicePlugin', 'addToGroup', [bridgeUDN, payload, isNewGroup], {
                useWindowMethod: bridgeUDN
            });
        },
        /**
         * removes a device/devices from a group
         * @param  {string} udn     unique device id
         * @param  {string} groupId unique group id
         * @return {promise}
         */
        removeFromGroup: function(bridgeUDN, groupID) {
            return callPlugin('SmartDevicePlugin', 'removeFromGroup', [bridgeUDN, groupID, false], {
                useWindowMethod: bridgeUDN
            });
        },
        /** removes a device/devices from a group
         * @param  {string} type
         * @param  {json} deviceInfo
         * @param  {json} stateData
         * @return {promise}
         */
        resetDevice: function(type, deviceInfo, stateData, callback) {
            var dataArray = [type, deviceInfo];

            if (stateData !== null) {
                dataArray.push(stateData);
            }
            return callPlugin('SmartDevicePlugin', 'resetDevice', dataArray, {
                callback: callback
            });
        },
        /**
         * set blob storage from device
         * @param {String} udn
         * @param {JSON} data
         * @param {function} callback
         * @return {promise}
         */
        setBlobStorageData: function(udn, data, callback) {

            return callPlugin('SmartDevicePlugin', 'setBlobStorageData', [udn, data], {
                callback: callback,
                useWindowMethod: udn
            });
        },
        /**
         * get blob storage from device
         * @param {String} udn
         * @param {JSON} data
         * @param {function} callback
         * @return {promise}
         */
        getBlobStorageData: function(udn, data,callback) {
            return callPlugin('SmartDevicePlugin', 'getBlobStorageData', [udn,data],{
                callback: callback,
                useWindowMethod: udn
            });
        },
        /**
         * configure Dimming Range.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        configureDimmingRange: function(deviceInfo) {
            return callPlugin('SmartDevicePlugin', 'configureDimmingRange', [deviceInfo]);
        },
        /**
         * check if Bulb devices are connected.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        adjustIcon: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'adjustIcon', [deviceInfo], {
                callback: callback
            });
        },
        /**
         * check if Bulb devices are connected.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        choosePhoto: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'choosePhoto', [deviceInfo], {
                callback: callback
            });
        },

        /**
         * check if Bulb devices are connected.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        takePicture: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'takePicture', [deviceInfo], {
                callback: callback
            });
        },
        /**
         * check if there is Netcam app installed
         * @return {promise}
         */
        checkNetcam: function() {
            return callPlugin('DevicePlugin', Constants.actions.CHECK_NET_CAM);
        },
        /**
         * launch Netcam app
         * @param  {string} item
         * @param  {json} deviceInfo
         * @param  {integer} extraParam
         * @return {promise}
         */
        launchNetcam: function(item, deviceInfo, extraParam) {
            var dataArray = [item, deviceInfo, extraParam];

            return callPlugin('DevicePlugin', Constants.actions.LAUNCH_NET_CAM_APP, dataArray);
        },
        /**
         * check if Bulb devices are connected.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        findBulb: function(deviceInfo) {
            return callPlugin('SmartDevicePlugin', 'findBulb', [deviceInfo]);
        },
        /**
         * remove device disconnected from device list.
         * @param  {json} deviceInfo
         * @return {promise}
         */
        hideDevice: function(deviceInfo) {
            return callPlugin('SmartDevicePlugin', 'hideDevice', [deviceInfo]);
        },
        /**
         * remove device disconnected from device list.
         * @param  {string} groupID
         * @return {promise}
         */
        hideGroup: function(groupID) {
            return callPlugin('SmartDevicePlugin', 'hideGroup', [groupID]);
        },
        /**
         * initiate App recovery mecahnism
         * @return {promise}
         */
        initiateAppRecovery: function() {
            return callPlugin('SmartDevicePlugin', 'appRecovery');
        },
        /**
         * set bulb type
         * @param  {json} deviceInfo
         * @param {function} callback
         * @return {promise}
         */
        setBulbType: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'setBulbType', [deviceInfo], {
                callback: callback
            });
        },
        /**
         * calibrate bulbs for dimmer
         * @param  {json} deviceInfo
         * @param {function} callback
         * @return {promise}
         */
        calibrateBulb: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'calibrate', [deviceInfo], {
                callback: callback
            });
        },
        configHushMode: function(deviceInfo, callback) {
            return callPlugin('SmartDevicePlugin', 'configureHushMode', [deviceInfo], {
                callback: callback
            });
        },
        /**
        * to check if bohnjour present in network
        * @param {function} callback
        * @return {promise}
        */
        checkBohnjour: function(callback){

            return callPlugin('SmartDevicePlugin', 'checkBohnjour',[], {
                 callback : callback
            });
        },

        /**
         * generat auth code
         * @param {function} callback
         * @return {promise}
         */
        getAuthCode: function(callback) {
            return callPlugin('WebServicePlugin', 'getAuthCode', [], {
                callback: callback
            });
        },
        /**
         * generat auth code
         * @return {promise}
         */
        openAlexa: function() {
            return callPlugin('DevicePlugin', '9106', []);
        },
        /**
         * Smart device uuid
         * @return {promise}
         */
        getUUID: function(callback) {
            return callPlugin('DevicePlugin', '6004', [],{
                callback: callback
            });
        },
        getRemoteAccess: function(callback) {
            return callPlugin('DevicePlugin', '6002', [],{
                callback: callback
            });
        },
        getAnalyticsData: function(callback) {
            return callPlugin('NativeUtilPlugin', '6006', [],{
                callback: callback
            });
        }
    };

    return exports;
});

//this is the base edit screen. load any additional edit view screens here. make sure to pass the newly created model to the views when instantiating them.
define('widgets/wemo_lighting/js/edit',['Cache', 'hbs!widgets/wemo_lighting/templates/edit','widgets/wemo_lighting/js/model', 'libs/smartDevicePlugin'],
    /** @exports summary view */
    function(Backbone, template, Model, smartDevicePlugin) {
        'use strict';

        var View = Backbone.View.extend({
            template: template,

            events: {
                'click #find_bulb': 'findBulb'
            },
            initialize: function(options) {
                options = options || {};
                _.extend(this, options);
                if (!this.sandbox) {
                    throw ('you must pass the sandbox to this view.')
                } else {
                    this.model = new Model.Model();
                    this.model.set(this.sandbox.model.toJSON());
                    this.model.baseObject = this.sandbox.model.baseObject;
                }
            },
            /**
             * two-way data binding
             */
            bindings: {

            },
            /**
             * localizes the template, and inserts it into the document fragment. Also binds the DOM events
             */
            render: function() {
                var html = this.i18n(this.template);
                this.$el.html(html);
                this.stickit();
                this.delegateEvents();
            },
            /**
             * put non-DOM event listeners here. These need to be removed when no longer necessary to free up memory (this.stopListening())
             */
            listen: function() {

            },
            /**
             * make a request to check if currect bulb device is connected
             */
            findBulb: function() {
                var info = {
                    'udn': this.sandbox.information.udn
                };

                smartDevicePlugin.findBulb(info).then(this.successChecking).fail(this.failChecking);
            },
            successChecking: function() {
                DEBUG && console.log('Success on checking Bulb device!!');
            },
            /**
             * action for failed request
             */
            failChecking: function() {
                DEBUG && console.log('Error on checking Bulb device!!');
            }
        });

        return View;
    });
define('widgets/wemo_lighting/main', ['text!widgets/wemo_lighting/css/main.css','widgets/wemo_lighting/js/model','widgets/wemo_lighting/js/tile','widgets/wemo_lighting/js/drawer','widgets/wemo_lighting/js/summary','widgets/wemo_lighting/js/settings','widgets/wemo_lighting/js/edit'],

    function(css, data, TileView, DrawerView, SummaryView, SettingsView, EditView) {

        'use strict';

        return {
            type: 'TileAndDrawer',
            styles: css,
            Tile: TileView,
            Drawer: DrawerView,
            Summary: SummaryView,
            Settings: SettingsView,
            Model: data.Model,
            config: {
                groupable: true,
                groupOn: ['productType', 'productName'],
                edit: {
                    View: EditView
                }
            }
        };

    });}());