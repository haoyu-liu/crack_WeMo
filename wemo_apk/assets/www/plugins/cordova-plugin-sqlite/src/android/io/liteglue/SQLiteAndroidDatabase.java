...
class SQLiteAndroidDatabase
{
    ...
    void open(File dbfile) throws Exception {
    	...
    	try {
        	//First we try to open the DB as we would like to set the following flag: NO_LOCALIZED_COLLATORS
        	mydb = SQLiteDatabase.openDatabase(dbfile.getAbsolutePath(), null, SQLiteDatabase.NO_LOCALIZED_COLLATORS | SQLiteDatabase.OPEN_READWRITE);
			
		} catch (SQLiteException e) {
			LogUtils.errorLog(TAG, "SQLiteException while opening DB: ", e);
			//DB does not exist does creating the DB.
