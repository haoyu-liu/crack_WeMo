...
(function ($) {
    function Scroller(elem, settings) {
        var m,
            ...
            ms = $.wemoTicker,
            e = elem,
			elm = (e.id=="set_cost_per") ? $(e).children('.cost_per_current_value') : $(e).children('.power_threshold_current_value'),			
            theme,
            lang,
            s = extend({}, defaults),
            pres = {},
        ...
        function generateWheelItems(i) {
            ...
            for (j in warr[i]) {				
                ...
                html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px; line-height:' + hi + 'px;"><div class="dw-i">';
					if (e.id=="set_cost_per") {
						(i==0) ? html+='$'+warr[i][j]+'.' :  html+=warr[i][j];
					} else {
						html+=warr[i][j];
					}
				html += '</div></div>';
        ...
        function read() {
            that.temp = ((input && that.val !== null && that.val != elm.val()) || that.values === null) ? s.parseValue(elm.text() || '', that) : that.values.slice(0);
			that.setValue(true);
        }
            ...
            if (s.display != 'inline') {
                ...
                that.tap($('.dwb-s span', dw), function () {
                    if (that.hide(false, 'set') !== false) {
						if ( $('.dwv').html().indexOf('$') == 0 ) {
							var val=$('.dwwl0 .dw-sel').attr('data-val')+
									$('.dwwl1 .dw-sel').attr('data-val')+
									$('.dwwl2 .dw-sel').attr('data-val')+
									$('.dwwl3 .dw-sel').attr('data-val');
							window.sessionStorage.setItem(sessionStore.DEVICE_EDIT_WEMO_COST_PER_KWH, val);	
							window.sessionStorage.removeItem(sessionStore.COST_PER_RESET, true);	
							var cur_sign = window.sessionStorage.getItem(sessionStore.DEVICE_EDIT_WEMO_CURRENCY_SYMBOL);
							editCostPer.fillCurrentValue('.cost_per_current_value', cur_sign+editCostPer.formatCost( val ));
						}
						else {							
							var power_threshold=parseInt($('.dw-sel').children().html());							
							window.sessionStorage.setItem(sessionStore.DEVICE_EDIT_WEMO_POWER_THRESHOLD, power_threshold*1000);
							...
							$('.power_threshold_current_value').text(power_threshold);					
						}
                    }
                });
    ...
    function calc(t, val, dir, anim, orig) {
		...
		var updateVal=$('.dw-i', t).eq(val).html();
		return updateVal;
    }
    ...
    $.fn.wemoTicker = function (method) {		
        extend(this, $.wemoTicker.shorts);		
        return init(this, method, arguments);
    };

    $.wemoTicker = $.wemoTicker || {
        /**
    ...
    $.scroller = $.scroller || $.wemoTicker;
    $.fn.scroller = $.fn.scroller || $.fn.wemoTicker;
