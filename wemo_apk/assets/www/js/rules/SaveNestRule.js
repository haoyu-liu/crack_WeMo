...
function saveNewRule_RULES_RULEDEVICES(arg, ruleObj) {
    ...
    function saveMotionRule(tx) {
        ...
        console.log(TAG + "ruleName" + ruleName);
        tx.executeSql('SELECT RuleID FROM RULES ORDER BY CAST(RuleID AS INT) DESC', [], querySuccess, errorCB("select rules save motion"));

        function querySuccess(tx, res) {
            console.log(TAG + "get rules query" + res);
            ...
            if (ruleObj.id > 0) {
                executeQuery = 'SELECT distinct a.RuleID, a.Name, a.Type, a.State, b.DeviceID, b.StartTime, b.RuleDuration, b.StartAction, b.EndAction, b.DayID, b.Value, b.SensorDuration, b.ZBCapabilityStart from RULES a inner join RULEDEVICES b on a.RuleID=b.RuleID where b.RuleID <> ' + ruleObj.id + ' order by a.Name COLLATE NOCASE';
            } else {
                executeQuery = 'SELECT distinct a.RuleID, a.Name, a.Type, b.DeviceID, a.State, b.StartTime, b.RuleDuration, b.StartAction, b.EndAction, b.DayID, b.Value, b.SensorDuration, b.ZBCapabilityStart from RULES a inner join RULEDEVICES b on a.RuleID=b.RuleID order by a.Name COLLATE NOCASE';
            }

            tx.executeSql(executeQuery, [],
                function (tx, results) {

                    console.log(TAG + "insert rules query" + res);
                    ...
                    var RuleDuration = "";
                    var awayState = "";
                    if (Object.keys(ruleObj.nests).length > 0) {
                        for (var sensor in ruleObj.nests) {

                            var productName = ruleObj.nests[sensor].ProductName || "";

                                awayState = ruleObj.nests[sensor].awayState;
                            ...
                            tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + sensor + '", "0", "-1", "' + ruleObj.When.StartTime + '","' + ruleObj.When.RuleDuration + '", "-1", "-1", "1","5","0","2","' + awayState + '", NULL,' + ruleObj.When.OnModeOffset + ',' + ruleObj.When.OffModeOffset + ', NULL,' + ruleObj.When.EndTime + ')');
                    ...
                    if (Object.keys(ruleObj.Devices).length > 0) {
                        for (device in ruleObj.Devices) {
                            ...
                            for (var daysCount1 = 0; daysCount1 < arrDays.length; daysCount1++) {
                                ...
                                } else if (ruleObj.When.EndSRSS != undefined) {
                                ...
                                var productName = ruleObj.Devices[device].ProductName;
                                ...
                                if(wemoUtil.isMakerDevice(device)){
                                    var makerUDN = wemoUtil.getMakerSwithUDN(device);
                                    tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + makerUDN + '", "0", "' + deCodeDays(arrDays[daysCount1]) + '", "' + StartTime + '","' + RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ', "' + ruleObj.Devices[device].SensorDuration + '","-1","-1","-1","' + awayState + '","-1",' + ruleObj.When.OnModeOffset + ',' + ruleObj.When.OffModeOffset + ',0,' + ruleObj.When.EndTime + ')');
                                } else if(device.match(/dimmer/gi)) {
                                    var dimmerZBStart = (ruleObj.Devices[device].StartAction === 0)? "0:0:0" : "1:0:0";
                                    tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + device + '", "0", "' + deCodeDays(arrDays[daysCount1]) + '", "' + StartTime + '","' + RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ', "' + ruleObj.Devices[device].SensorDuration + '","-1","-1","-1","' +  dimmerZBStart + '","-1",' + ruleObj.When.OnModeOffset + ',' + ruleObj.When.OffModeOffset + ',0,' + ruleObj.When.EndTime + ')');
                                } else {
                                    tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + device + '", "0", "' + deCodeDays(arrDays[daysCount1]) + '", "' + StartTime + '","' + RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ', "' + ruleObj.Devices[device].SensorDuration + '","-1","-1","-1","' + awayState + '","-1",' + ruleObj.When.OnModeOffset + ',' + ruleObj.When.OffModeOffset + ',0,' + ruleObj.When.EndTime + ')');
                                }
                            }
                        }
                    }
    ...
    function successfullySaved() {
        ...
        if (saved) {
            ...
            setTimeout(function () {
                pushScheduleSensor(ruleObj);
            }, 1000);
        }
    }

    function pushScheduleSensor(rule) {
        var sensorArrayObj = Object.keys(rule.Devices);
    ...
    function rulesEquals(rule1, rule2) {
        ...
        for(var nest in rule1.nests){
            awayState = rule1.nests[nest].awayState;
        }
        ...
        if ((rule1.When.StartTime == rule2.StartTime) &&
            ...
            (awayState == rule2.ZBCapabilityStart)) {
    ...
    function ruleConflicts(rule2, rule1) {
        ...
        if(wemoUtil.isMakerDevice(targetDeviceID)){
            targetDeviceID = wemoUtil.getOriginalMakerUDN(targetDeviceID);
        }
        ...
        for(nest in rule2.nests){
            if(rule2.nests[nest].awayState !== rule1.ZBCapabilityStart){
                return false
            }
        }
        ...
        if (msgType == "conflict_rule") {
            ...
            switch (ruleType) {
                ...
                case sessionStore.MOTION_CONTROLLED:
                    ...
                    break;
                case sessionStore.EVENT_CONTROLLED:
                    ruleConflictLocalizedmsg = wemo.items['RuleConflictNest'];
                    temparr = ruleConflictLocalizedmsg.split("%@");
                    var msgText = temparr[0] + " " + ruleName + ". " + temparr[1];
                    break;
            }
...
//        var arrSensCurRule = ruleObj.SelectedSensors.split(",");
        var arrSensCurRule = [];
        if (Object.keys(rule.nests).length > 0) {
            for (sensor in rule.nests) {
        ...
        for (var i = 0; i < ruleRow.length; i++) {
            ...
            if (tmpRule != undefined) {
                if (tmpRule.Type == sessionStore.EVENT_CONTROLLED && (tmpRule.DayID).toString() == "-1") {
