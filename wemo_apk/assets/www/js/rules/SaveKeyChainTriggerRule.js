...
var TAG='SaveKeyChainTriggerRule.js: ';
...
function saveNewRule_RULES_RULEDEVICES(arg, ruleObj) {
	...
	ruleObj.RuleType = sessionStore.MOTION_CONTROLLED;
    window.sessionStorage.setItem(sessionStore.RULE_TYPE, JSON.stringify(sessionStore.MOTION_CONTROLLED));
	
	var db = window.sqlitePlugin.openDatabase(SQLColumns.DB_NAME, SQLColumns.DB_VERSION, SQLColumns.DB_NAME, 4500000);
    db.transaction(saveKeyChainTriggerRule, errorCB, successCB);
    db.transaction(getRules, errorCB, successfullySaved);

    function saveKeyChainTriggerRule(tx) {
    	var ruleName = ruleObj.RuleName.replace(/\"/g,'&quot;');
    	wemoUtil.infoLog(TAG,"saveKeyChainTriggerRule_ruleName" + ruleName);
    	...
    	function querySuccess(tx, res) {
    		...
    		if (ruleObj.id > 0) {
    			executeQuery = 'SELECT distinct a.RuleID, a.Name, a.Type, a.State, b.DeviceID, b.StartTime, b.RuleDuration, b.StartAction, b.EndAction, b.DayID, b.Value, b.SensorDuration from RULES a inner join RULEDEVICES b on a.RuleID=b.RuleID where b.RuleID <> ' + ruleObj.id + ' order by b.RuleID';
    		} else {
    			executeQuery = 'SELECT distinct a.RuleID, a.Name, a.Type, b.DeviceID, a.State, b.StartTime, b.RuleDuration, b.StartAction, b.EndAction, b.DayID, b.Value, b.SensorDuration from RULES a inner join RULEDEVICES b on a.RuleID=b.RuleID order by b.RuleID';
    		}
    		tx.executeSql(executeQuery, [],
    			...
    			if (ruleObj.id > 0) {
    				...
    				tx.executeSql("DELETE FROM DEVICECOMBINATION WHERE RuleID=" + ruleObj.id, [], function () {
    					wemoUtil.infoLog(TAG,"RuleCombination with ID : " + ruleObj.id + " is deleted");
    				}, errorCB);
    			...
    			if (Object.keys(ruleObj.Devices).length > 0) {
    				...
    				for (device in ruleObj.Devices) {
    					...
    					for (var daysCount1 = 0; daysCount1 < arrDays.length; daysCount1++) {
    						...
    						wemoUtil.infoLog(TAG,"before Push First :: " + '(' + ruleObj.id + ',"' + device + '", "0", ' + deCodeDays(arrDays[daysCount1]) + ', "' + StartTime + '","' + ruleObj.When.RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ', "' + ruleObj.Devices[device].SensorDuration + '","-1","' + "" + '","-1")');
    						if(isLedDevice)
    							tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + bridgeUDN + '", "0", "' + deCodeDays(arrDays[daysCount1]) + '", "' + StartTime + '","' + ruleObj.When.RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ' , -1,"-1","-1","-1","","",'+ruleObj.When.OnModeOffset+','+ruleObj.When.OffModeOffset+',-1,'+ruleObj.When.EndTime+')');
    						else
    							tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart , ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + device + '", "0", "' + deCodeDays(arrDays[daysCount1]) + '", "' + StartTime + '","' + ruleObj.When.RuleDuration + '", ' + ruleObj.Devices[device].StartAction + ', ' + ruleObj.Devices[device].EndAction + ', -1,"-1","-1","-1","","",'+ruleObj.When.OnModeOffset+','+ruleObj.When.OffModeOffset+',-1,'+ruleObj.When.EndTime+')');
    					}
    				}
    			}
    			...
    			if (Object.keys(ruleObj.Sensors).length > 0) {
    				for (sensor in ruleObj.Sensors) {
    					wemoUtil.infoLog(TAG,"Sensor Id"+ sensor+"Sensor state: "+ ruleObj.Sensors[sensor]['state']+ "  <<<>>>> RuleObj --->>  "+ JSON.stringify(ruleObj));

    					var productName = "";
    					wemoUtil.infoLog(TAG,"before Push Second :: " + '(' + ruleObj.id + ',"' + sensor + '", "0", ' + '-1' + ', "' + ruleObj.When.StartTime + '","' + ruleObj.When.RuleDuration + '", ' + '-1' + ', ' + '-1' + ', "' + "-1" + '","0","' + ruleObj.Sensors + '","2")');
    					tx.executeSql('INSERT INTO RULEDEVICES (RuleID, DeviceID, GroupID, DayID, StartTime,RuleDuration, StartAction, EndAction, SensorDuration,Type,Value,Level,ZBCapabilityStart, ZBCapabilityEnd,OnModeOffset,OffModeOffset,CountdownTime,EndTime) VALUES (' + ruleObj.id + ',"' + sensor + '", "0", "-1", "' + ruleObj.When.StartTime + '","' + ruleObj.When.RuleDuration + '", "' + ruleObj.Sensors[sensor]['state'] + '", "-1", "-1","0","' + ruleObj.Sensors[sensor]['state'] + '","' + ruleObj.Sensors[sensor]['state'] + '","","",'+ruleObj.When.OnModeOffset+','+ruleObj.When.OffModeOffset+',-1,'+ruleObj.When.EndTime+')');
    ...
    function pushScheduleSensor(params, rule){
     ...
     for (var inx = 0; inx < devicesLength ; inx++) {
      ...
      for (var j =0; j < params.length ; j++){
    	  if(udn && udn!=null && params[inx] && params[inx]!=null && params[inx].udn && params[inx].udn!=null){
    		  wemoUtil.infoLog(TAG,"saveKeyChainTriggerRule: " + udn + ", params_udn: " + params[j].udn);
    		  if (udn == params[j].udn){
    ...
    function getRules(tx) {
        ...
        tx.executeSql('SELECT distinct a.RuleID, a.Name, a.Type, a.State, b.StartTime, b.RuleDuration, b.DayID, b.RuleDuration, b.StartTime, b.DeviceID, b.StartAction, b.EndAction, b.SensorDuration, b.Value, b.Level from RULES a inner join RULEDEVICES b on a.RuleID=b.RuleID order by b.RuleID', [],
            function (tx, results) {
    ...
    function deviceUsed(rule, devid) {
    	if (Object.keys(rule.Devices).length > 0) {
    		for (device in rule.Devices) {
    			wemoUtil.infoLog(TAG,"deviceUsed() :: device--> "+ device + "  && devid--> "+ devid);
    			if (device == devid) {
    				return true;
    			}
    		}
    ...
    function rulesEquals(rule1, rule2) {
      ...
      wemoUtil.infoLog(TAG,"rulesEquals():: rule1--> "+ JSON.stringify(rule1));
      wemoUtil.infoLog(TAG,"rulesEquals():: rule2--> "+ JSON.stringify(rule2));
    ...
    function ruleConflicts(rule2, rule1) {
      wemoUtil.infoLog(TAG,"ruleConflicts():: rule1--> "+ JSON.stringify(rule1));
      wemoUtil.infoLog(TAG,"ruleConflicts():: rule2--> "+ JSON.stringify(rule2));
    ...
    function verifyRuleDuplicatesAndConflicts(rule, ruleRow) {
        ...
        wemoUtil.infoLog(TAG,"Check for rule for dupilacation...");
        wemoUtil.infoLog(TAG,"New rule name: " + rule.RuleName);
        wemoUtil.infoLog(TAG,"verifyRuleDuplicatesAndConflicts() rule--> "+JSON.stringify(rule));
        wemoUtil.infoLog(TAG,"verifyRuleDuplicatesAndConflicts() ruleRow--> "+JSON.stringify(ruleRow) + "  "+ ruleRow);
        var sensorRulesInfo ={};
        	...
        	if(tmpRuleRow.DayID == -1){
        		if (Object.keys(rule.Sensors).length > 0) {
        			for (sensor in rule.Sensors) {
        				if(wemoUtil.getRuleClass(rule.RuleType) == wemoUtil.getRuleClass(tmpRuleRow.Type) && tmpRuleRow.DeviceID == sensor){
        					if(tmpRuleRow.StartAction == rule.Sensors[sensor].state){
        						sensorRulesInfo[tmpRuleRow.RuleID] = true ;
        					}
        				}
        			}
        		}
        ...
        for (var i = 0; i < ruleRow.length; i++) {
            ...
            wemoUtil.infoLog(TAG,"verifyRuleDuplicatesAndConflicts() tmpRule--> "+JSON.stringify(tmpRule));
            if (tmpRule != undefined) {
                if (wemoUtil.getRuleClass(rule.RuleType) == wemoUtil.getRuleClass(tmpRule.Type)) {
                        ...
                        else if(rule.Devices[tmpRule.DeviceID] != undefined){
                        	fname = rule.Devices[tmpRule.DeviceID].FName;
                        }
                        if (rulesEquals(rule, tmpRule) && sensorRulesInfo[tmpRule.RuleID] == true) {
                        	nativeUtilPlugin.execute(null, null, actions.HIDE_SPLASH_ON_LOADING, []);
                        	wemoUtil.infoLog(TAG,JSON.stringify(tmpRule));
                        	wemoUtil.infoLog(TAG,"Found new rule duplication with: " + tmpRuleName);
                        	showConflictDuplicateMessage(tmpRule.Name, "duplicate_rule", fname, null, null, null, null);
                        	return true;
                        }
                        if (ruleConflicts(rule, tmpRule) && sensorRulesInfo[tmpRule.RuleID] == true) {
                        	if (isWholeDay(rule)) {
                        		nativeUtilPlugin.execute(null, null, actions.HIDE_SPLASH_ON_LOADING, []);
                        		wemoUtil.infoLog(TAG,JSON.stringify(tmpRule));
                        		wemoUtil.infoLog(TAG,"Found new rule whole-day conflict with: " + tmpRuleName);
                        		showConflictDuplicateMessage(tmpRule.Name, "conflict_day__rule", fname, null, null, null, null);
                        		return true;
                        	}
                        	else {
                        		wemoUtil.infoLog(TAG,JSON.stringify(tmpRule));
                        		wemoUtil.infoLog(TAG,"Found new rule conflict with: " + tmpRuleName);
