...
var TAG = "sensors_rule_when.js: ";
var UP_TF_IS24 = false;//UP-user preferences, TF-time format

$(document).bind('pageinit', function() {
	sensors_rule_when.initialize();
});

var sensors_rule_when = {
		...
		onDeviceReady : function() {
			...
			$(".drawer_offset div").on('tap', function(e) {
				...
				if ( !$(this).hasClass("custom") ) {
					...
					var time = $('input[name='+name_radio_btn+']:checked').val() == wemo.items["SunRise"] ? sensors_rule_when.timeSR_ : sensors_rule_when.timeSS_;
					var value = $(this).attr("value");
					
					var itemTitle = $('input[name='+name_radio_btn+']:checked').closest(".srss_items").find(".more_li_text_div");
					if ( value != "0" ) {
						itemTitle.empty().html( sensors_rule_when.getSRSSheader( value, sr_or_ss ) );
					} else {
					...
					subHeaderTime.empty().html( sensors_rule_when.getSRSSsubHeaderWithOffset( time, value ) );
					if ( value != "0" ) {
					...
					switch ( tab ) {
						case wemo.items["TabStartTime"] : {				
							...
							$("#custom_offset_selector_start").BelkinPicker({type: "offset", curval: "00:20 before"}, "show", function (val) {					
								sensors_rule_when.setOffsetFromPicker( "sensors_start_srss", wemo.items["TabStartTime"], val, ruleObj);
						...
						case wemo.items["TabEndTime"] : {
							...
							$("#custom_offset_selector_end").BelkinPicker({type: "offset", curval: "00:20 before"}, "show", function (val) {					
								sensors_rule_when.setOffsetFromPicker( "sensors_end_srss", wemo.items["TabEndTime"] , val, ruleObj);
            ...
            if (ruleObj.When.StartSRSS != undefined && ruleObj.When.EndSRSS != undefined){
            	sensors_rule_when.getLocationData(ruleObj);
            ...
            if (ruleObj.When.StartSRSS != undefined){
				...
				if ( ruleObj.When.OnModeOffset != "0" ) {	
					setTimeout(function(){
						...
						var time = $('input[name=sensors_start_srss]:checked').val() == wemo.items["SunRise"] ? sensors_rule_when.timeSR_ : sensors_rule_when.timeSS_;
						var xx = sensors_rule_when.getSRSSsubHeaderWithOffset( time, ruleObj.When.OnModeOffset );
						console.log("xx "+xx);
						...
						itemHeader.empty().html( sensors_rule_when.getSRSSheader( ruleObj.When.OnModeOffset, sr_or_ss ) );
						
						var drawer = sr_or_ss == wemo.items["SunRise"] ? $(".startTime .srss .sr_offset") : $(".startTime .srss .ss_offset");
						sensors_rule_when.checkDivInDrawer( drawer, ruleObj.When.OnModeOffset );	
					}, 100);
				} 
            } else {
				$(".startTime .srss").hide();
            ...
            if (ruleObj.When.EndSRSS != undefined){            	
				...
				if ( ruleObj.When.OffModeOffset != "0" ) {
					setTimeout(function(){
						...
						var time = $('input[name=sensors_end_srss]:checked').val() == wemo.items["SunRise"] ? sensors_rule_when.timeSR_ : sensors_rule_when.timeSS_;
						subHeaderTime.empty().html( sensors_rule_when.getSRSSsubHeaderWithOffset( time, ruleObj.When.OffModeOffset ) );
						subHeader.find("span").removeClass("link_gray").addClass("link_green");		
						...
						itemHeader.empty().html( sensors_rule_when.getSRSSheader( ruleObj.When.OffModeOffset, sr_or_ss ) );
														
						var drawer = sr_or_ss == wemo.items["SunRise"] ? $(".endTime .srss .sr_offset") : $(".endTime .srss .ss_offset");				
						sensors_rule_when.checkDivInDrawer( drawer, ruleObj.When.OffModeOffset );
					}, 100);
				} 
            } else {
				$(".endTime .srss").hide();
			...
			$('.startTime').find('#startSunrise, #startSunset').on('click', function(){  				
				...
				sensors_rule_when.selectSRSS(ruleObj, "startSRSS");
				...
				$("input[name='sensors_start_srss']").each(function(){			
					if( !$(this).is(':checked') ){
						...
						var time = $(this).val() == wemo.items["SunRise"] ? sensors_rule_when.timeSR_ : sensors_rule_when.timeSS_;
						...
						subHeader.find("span").removeClass("link_green").addClass("link_gray");
						subHeaderTime.empty().html( sensors_rule_when.getSRSSsubHeaderWithOffset( time, "0" ) );
			...
			$('.endTime').find('#endSunrise, #endSunset').on('click', function(){  				
				...
				sensors_rule_when.selectSRSS(ruleObj, "endSRSS");
			...
			$("#schedule div").on("tap", function() {
				if ($(this).hasClass("div_bg_green") && ( sensors_rule_when.getNumberOfSelectedDays() != 1 ) ) {
			...
			$("#start_sensor_navbar ul li").on("click",function() {
				...
				$("#start_sensor_navbar ul li").each(function() {
					...
					if ($(this).children("a").hasClass("ui-btn-active")){
						$("."+tabID).show();
						if (tabID=="startSRSS"){
							sensors_rule_when.selectSRSS(ruleObj,tabID);
							sensors_rule_when.getLocationData(ruleObj);
						}else{
			...
			/**************** Open Sunrise / Sunset  for End time ******************/
			/*
			$("#end_sensor_navbar ul li").on("click",function() {
				$("#end_sensor_navbar ul li").each(function() {
					var tabID=$(this).children("a").attr("data-tab-class");
					var selectedWhen=$(this).children("a").attr("data-when");
					if ($(this).children("a").hasClass("ui-btn-active")){
						//alert(" Show Mode  Start  :: "+ruleObj.When.StartSRSS+'  End ::  '+ruleObj.When.EndSRSS);
						$("."+tabID).show();
						if (tabID=="endSRSS"){
							sensors_rule_when.selectSRSS(ruleObj,tabID);
							sensors_rule_when.getLocationData(ruleObj);
			...
			*/
	        ...
	        $('#start_time').on('change', function(){
            	$('#end_time').val(sensors_rule_when.update_end_time($('#start_time').val()));
            	wemoUtil.setJSONObjectToSessionStorage(sessionStore.RULE_NEW,ruleObj);
	        });
			...
			/*$("#sensors_edit_days").on("tap", function(){
				...
				// Selected
				//alert("sunriseState::  "+sunriseState);
				//-------------------Start/End SRSS--------------------
		    	if (startSunriseState == false){
					$("input[class='sensors_startSunrise']").each(function() {
						var startSelected_state = $("input[class='sensors_startSunrise']").is(":checked");
						$("[name='sensors_startSunrise_method']").checkboxradio("refresh");
						if ($(this).is(':checked')) {
							ruleObj.When.StartSRSS = $(this).val();
							//alert("ruleObj.When.StartSRSS:  "+ruleObj.When.StartSRSS);
						}
					});
				...
				if (endSunriseState == false) {
					$("input[class='sensors_endSunrise']").each(function() {
						var endSelected_state = $("input[class='sensors_endSunrise']").is(":checked");
						$("[name='sensors_endSunrise_method']").checkboxradio("refresh");
						if ($(this).is(':checked')) {
							ruleObj.When.EndSRSS = $(this).val();
//							alert("ruleObj.When.EndSRSS:  "+ruleObj.When.EndSRSS);
						}
					});
				...
				//-------------------Start/End SRSS--------------------
					...
					if($("#sensorAllDay").val()==="on"){
	             	...
	             	ruleObj.When.EndSRSS = undefined;
				}
...
/*										if(ruleObj.When.StartSRSS == "Sunrise"){
		    ...
		    if (ruleObj.When.StartSRSS != undefined) {
			    ...
			    $("#startSunset").attr("checked","checked")
		    	...
		    	$("#startSunrise").attr("checked", "checked");
				$("input[type='radio']").checkboxradio("refresh");
		    }
		    ...
		    if(ruleObj.When.EndSRSS != undefined){
		    	$("#endSunset").attr("checked","checked");
		    	$("input[type='radio']").checkboxradio("refresh");
		    }
		    
			if (ruleObj.When.EndSRSS != undefined) {
				//alert("onDeviceReady = ruleObj.When.EndSRSS:  "+ruleObj.When.EndSRSS);
			    $("#endSunrise").attr("checked","checked");
		    ...
		    //-------------------End SRSS--------------------
			    ...
			    //Save button click
	            $("#sensors_rule_when_save_btn").on('pointerdown', function(){
					...
					if($("#sensorAllDay").val()=="on"){
						...
						if(startSunriseState===false){
							$("input[class='timer_startSunrise']").each(function(){
								var startSelected_state = $("input[class='timer_startSunrise']").is(":checked");
...
//									alert($(this).val());
						...
						if(endSunriseState===false){
							$("input[class='timer_endSunrise']").each(function(){
								var endSelected_state = $("input[class='timer_endSunrise']").is(":checked");
...
//					 window.sessionStorage.setItem(sessionStore.RULES_DATA, JSON.stringify(ruleObj));
		...
		getLocationData : function(ruleObj){
			...
			function getLatLongFromDB_SuccessCB(success){
				 ...
				 sensors_rule_when.showTimeTodaySRSS(ruleObj);
		    	 console.log('Successfully - getLatLongFromDB_SuccessCB '+success);
		    }
		...
		bindEvents : function() {
			...
			document.addEventListener('online', sensors_rule_when.onOnline, false);
			document.addEventListener('offline', sensors_rule_when.onOffline, false);
			document.addEventListener("backbutton", sensors_rule_when.onBackPress, false);
			document.addEventListener("pause", sensors_rule_when.onPause, false);
			document.addEventListener("resume", sensors_rule_when.onResume, false);
		},
						...
						function (params) {
							if (params.length > 0) {					
								...
								$.each(params, function (index) {
									...
									if ( today == day ) {
										sensors_rule_when.timeSR_ = tempData.split(":")[0];
										sensors_rule_when.timeSS_ = tempData.split(":")[1];	
										
										if(sensors_rule_when.timeSS_ > timestamp.MAX_HOUR){
										...
										$(".timeSR").empty().html( wemoUtil.convertMillisecondsToTF(sensors_rule_when.timeSR_, UP_TF_IS24) );
										$(".timeSS").empty().html( wemoUtil.convertMillisecondsToTF(sensors_rule_when.timeSS_, UP_TF_IS24) );
									}
								});
							}
							else
