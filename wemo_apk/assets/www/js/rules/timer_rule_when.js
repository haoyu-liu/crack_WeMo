...
var timer_rule_when = {
    ...
    onDeviceReady: function () {
		...
		if (ruleObj.When.StartSRSS != undefined && ruleObj.When.EndSRSS != undefined) {
            timer_rule_when.getLocationData(ruleObj);
		...
		$(".drawer_offset div").on('tap', function(e) {
			...
			if ( !$(this).hasClass("custom") ) {
				...
				var time = $('input[name='+name_radio_btn+']:checked').val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
				var value = $(this).attr("value");
				
				var itemTitle = $('input[name='+name_radio_btn+']:checked').closest(".srss_items").find(".more_li_text_div");
				if ( value != "0" ) {
					itemTitle.empty().html( timer_rule_when.getSRSSheader( value, sr_or_ss ) );
				} else {
				...
				subHeaderTime.empty().html( timer_rule_when.getSRSSsubHeaderWithOffset( time, value ) );
				if ( value != "0" ) {
				...
				setTimeout (function () {
					...
					case wemo.items["TabStartTime"] : {				
						...
						$("#custom_offset_selector_start").BelkinPicker({type: "offset", curval: "00:20 before"}, "show", function (val) {					
							timer_rule_when.setOffsetFromPicker( "rad_srss_start", wemo.items["TabStartTime"], val, ruleObj);
					...
					case wemo.items["TabEndTime"] : {
						...
						$("#custom_offset_selector_end").BelkinPicker({type: "offset", curval: "00:20 before"}, "show", function (val) {					
							timer_rule_when.setOffsetFromPicker( "rad_srss_end", wemo.items["TabEndTime"] , val, ruleObj);
		...
		$('.startTime').find('#startSunrise, #startSunset').on('tap', function(){
			ruleObj.When.StartSRSS = $(this).val();
            ruleObj.When.OnModeOffset = "0";
			timer_rule_when.selectSRSS(ruleObj, "startSRSS");
			...
			$("input[name='rad_srss_start']").each(function(){
				if( !$(this).is(':checked') ){
					...
					var time = $(this).val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
					...
					subHeader.find("span").removeClass("link_green").addClass("link_gray");
					subHeaderTime.empty().html( timer_rule_when.getSRSSsubHeaderWithOffset( time, "0" ) );
		...
		$('.endTime').find('#endSunrise, #endSunset').on('click', function(){  				
			...
			timer_rule_when.selectSRSS(ruleObj, "endSRSS");
			if ($(".startTime .drawer_offset").is(":visible")) {
				$(".startTime .drawer_offset").slideToggle("slow"); 
			}
			if ($(".endTime .drawer_offset").is(":visible")) {
...
/*
        if (ruleObj.When.StartSRSS != undefined) {            
            ...
            $('#start_time').empty().val(ruleObj.When.StartSRSS);
        }*/
...
/*
        if (ruleObj.When.EndSRSS != undefined) {
            ...
            $('#end_time').empty().val(ruleObj.When.EndSRSS);
        }*/
        ...
        $("#schedule div").on("tap", function () {
            if ($(this).hasClass("div_bg_green") && ( timer_rule_when.getNumberOfSelectedDays() != 1 ) ) {
        ...
        $("#toggleTS_startTime div").on("tap", function () {
            ...
            if (!$(this).hasClass("active")) {
                ...
                if (tabID == "startSRSS") {
					...
					$(".startTime .srss").show();
					setTimeout(function () {
                        timer_rule_when.selectSRSS(ruleObj, tabID);
                        timer_rule_when.getLocationData(ruleObj);
					}, 600);
                } else {
                    if ($(".startTime .drawer_offset").is(":visible")) {
						$(".startTime .drawer_offset").hide();
					}
        ...
        $('#start_time').on('change', function () {
            $('#end_time').val(timer_rule_when.update_end_time($('#start_time').val()));
            wemoUtil.setJSONObjectToSessionStorage(sessionStore.RULE_NEW, ruleObj);
        });
        ...
        $("#timer_rule_when_cancel_btn").on('click', function () {
            if (ruleObj.SelectedDayOrRange == undefined) {
        ...
        $("#timer_rule_when_save_btn").on('click', function () {
                            ...
                            $.each(params, function (index) {
                                ...
                                for (var day = 0; day < arrSelectedDays.length; day++) {
                                    ...
                                    if (arrSelectedDays[day] == SRSSDay) {
                                        ...
                                        if ( (ruleObj.When.StartSRSS === ruleObj.When.EndSRSS) && (ruleObj.When.StartTime > timestamp.MAX_HOUR || ruleObj.When.EndTime > timestamp.MAX_HOUR )) {
                                            //alert("same");
        ...
        /*var startSunriseState;
         ...
         //Save button click
         $("#timer_rule_when_save_btn").on('pointerdown', function(){
    ...
    selectSRSS: function (ruleObj, navBar) {
        if (ruleObj.When.StartSRSS == undefined && ruleObj.When.EndSRSS == undefined) {
            if (navBar == "startSRSS") {
                $("#startSunset").prop("checked", true);
                ruleObj.When.StartSRSS = $("#startSunset").attr("value");
				ruleObj.When.OnModeOffset = "0";
            } else {
                $("#endSunset").prop("checked", true);
                ruleObj.When.EndSRSS = $("#endSunset").attr("value");
				ruleObj.When.OffModeOffset = "0";
            }
        } else if (ruleObj.When.StartSRSS != undefined && ruleObj.When.EndSRSS == undefined) {
			...
			if (navBar == "startSRSS") {
                $("input[name=rad_srss_end]").each(function () {
					//if( $(this).is(':checked') ){ 
						var itemTitle = $(this).closest(".srss_start").find(".more_li_text_div");
						var subHeader = $(this).closest(".srss_start").find(".more_li_subHeader");
						var subHeaderTime =  $(this).closest(".srss_start").find(".more_li_subHeader .timeSRSS");
						var time = $(this).val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
				...
				console.log("navBar == 'endSRSS'");
                $("input[name=rad_srss_start]").each(function () {
					//if( $(this).is(':checked') ){	
						var itemTitle = $(this).closest(".srss_end").find(".more_li_text_div");
						var subHeader = $(this).closest(".srss_end").find(".more_li_subHeader");
						var subHeaderTime =  $(this).closest(".srss_end").find(".more_li_subHeader .timeSRSS");
						var time = $(this).val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
    ...
    getLocationData: function (ruleObj) {
        ...
        function getLatLongFromDB(tx) {
            errorTracker = "getLatLongFromDB()";
            //tx.executeSql("DELETE FROM LOCATIONINFO", [], function(){console.log("LOCATIONINFO is deleted");}, function (){console.log("Error delete info ");});
        ...
        function getLatLongFromDB_SuccessCB(success) {
			...
			timer_rule_when.showTimeTodaySRSS(ruleObj);
            console.log('Successfully - getLatLongFromDB_SuccessCB ' + success);
        }
    ...
    bindEvents: function () {
        ...
        document.addEventListener('online', timer_rule_when.onOnline, false);
        document.addEventListener('offline', timer_rule_when.onOffline, false);
        document.addEventListener("backbutton", timer_rule_when.onBackPress, false);
        document.addEventListener("pause", timer_rule_when.onPause, false);
        document.addEventListener("resume", timer_rule_when.onResume, false);
    },
					...
					function (params) {
						...
						if (params.length > 0) {					
							...
							$.each(params, function (index) {
								...
								if ( today == day ) {
									timer_rule_when.timeSR_ = tempData.split(":")[0];
									timer_rule_when.timeSS_ = tempData.split(":")[1];
									
									if(timer_rule_when.timeSS_ > timestamp.MAX_HOUR){
									...
									$(".timeSR").empty().html( wemoUtil.convertMillisecondsToTF(timer_rule_when.timeSR_, UP_TF_IS24) );
									$(".timeSS").empty().html( wemoUtil.convertMillisecondsToTF(timer_rule_when.timeSS_, UP_TF_IS24) );
								}
								if (ruleObj.When.StartSRSS != undefined) {
									if (ruleObj.When.OnModeOffset != undefined) {
										$("#toggleTS_startTime div").removeClass("active clock_not_active sun_not_active"); //attr class erased
										$("#toggleTS_startTime div").children().removeClass("time_active sun_active clock_not_active sun_not_active"); //attr class erased 
										$("#toggleTS_startTime .sun").addClass("active");
										$("#toggleTS_startTime .time").addClass("clock_not_active");
										...
										if ( ruleObj.When.OnModeOffset != "0" ) {				
											...
											var time = $('input[name=rad_srss_start]:checked').val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
											subHeaderTime.empty().html( timer_rule_when.getSRSSsubHeaderWithOffset( time, ruleObj.When.OnModeOffset )  );
											subHeader.find("span").removeClass("link_gray").addClass("link_green");				
											...
											itemHeader.empty().html( timer_rule_when.getSRSSheader( ruleObj.When.OnModeOffset, sr_or_ss ) );
											
											var drawer = sr_or_ss == wemo.items["SunRise"] ? $(".startTime .srss .sr_offset") : $(".startTime .srss .ss_offset");
											timer_rule_when.checkDivInDrawer( drawer, ruleObj.When.OnModeOffset );	
										} 
									} 
								}
								
								if (ruleObj.When.EndSRSS != undefined) {
									if (ruleObj.When.OffModeOffset != undefined) {
										$("#toggleTS_endTime div").removeClass("active clock_not_active sun_not_active"); //attr class erased
										$("#toggleTS_endTime div").children().removeClass("time_active sun_active clock_not_active sun_not_active"); //attr class erased 
										$("#toggleTS_endTime .sun").addClass("active");
										$("#toggleTS_endTime .time").addClass("clock_not_active");
										...
										if ( ruleObj.When.OffModeOffset != "0" ) {
											...
											var time = $('input[name=rad_srss_end]:checked').val() == wemo.items["SunRise"] ? timer_rule_when.timeSR_ : timer_rule_when.timeSS_;
											subHeaderTime.empty().html(  timer_rule_when.getSRSSsubHeaderWithOffset( time, ruleObj.When.OffModeOffset ) );
											subHeader.find("span").removeClass("link_gray").addClass("link_green");		
											...
											itemHeader.empty().html( timer_rule_when.getSRSSheader( ruleObj.When.OffModeOffset, sr_or_ss ) );
																			
											var drawer = sr_or_ss == wemo.items["SunRise"] ? $(".endTime .srss .sr_offset") : $(".endTime .srss .ss_offset");				
											timer_rule_when.checkDivInDrawer( drawer, ruleObj.When.OffModeOffset );
										} 
									} 
								}
							});
	...
	getSRSSsubHeaderWithOffset: function( timeSRSS, offset ) {
		...
		switch ( sign ) {
			case wemo.items["Before"]: console.log("beforeeeeee"); break;
			case wemo.items["After"]: console.log("afterrrr"); break;
			default: console.log("defaulttttt"); break;
		}
		switch ( sign ) {
